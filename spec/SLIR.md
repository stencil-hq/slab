...
`style=`, `key=`, `animate=`, and `transition=` do not appear in attribute
runs: styles are folded at compile time, keys become `node_key`, and motion
uses the animation fields. `act`, `field`, and `submit` become signal entries;
`each` is `Num(parameter index)` on an `Each` node.
