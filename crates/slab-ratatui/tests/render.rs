...
fn click_emits_signal() {
	...
	assert!(state.handle_event(&mouse(MouseEventKind::Down(MouseButton::Left), col, row), area));
	assert!(state.handle_event(&mouse(MouseEventKind::Up(MouseButton::Left), col, row), area));
	...
	assert!(state.drain_signals().is_empty(), "drain must empty the queue");
}
...
fn mouse_outside_area_is_dropped() {
	...
	let hit = state.handle_event(&mouse(MouseEventKind::Down(MouseButton::Left), col, row), area);
	let _ = hit; // translated relative to the shifted origin; must not panic
	assert!(!state.handle_event(&mouse(MouseEventKind::Down(MouseButton::Left), 2, row), area));
}
