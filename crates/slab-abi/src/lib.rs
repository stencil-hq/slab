...
pub unsafe extern "C" fn slab_request(handle: u32, ptr: *const u8, len: usize) -> *mut u8 {
    ...
    let response = SESSIONS
        .with_borrow_mut(|registry| registry.live.get_mut(&handle).map(|s| s.request(line)));
    match response {
...
mod tests {
    ...
    fn session_compiles_source_and_renders_cells() {
        ...
        request(
            handle,
            r#"{"method":"env.set","params":{"width":320,"height":96,"client":"tui"}}"#,
        );
        let cells = request(
            handle,
            r#"{"method":"render.cells","params":{"plain":true}}"#,
        );
    ...
    fn malformed_input_stays_inside_the_protocol() {
        ...
        assert!(
            empty.contains("-32700"),
            "an empty body is a parse error, not a crash: {empty}"
        );
        slab_session_free(handle);
    }
}
