...
use slab_syntax::lex::{TokKind, lex};
...
pub struct Index {
    ...
    /// List-valued exported-def field name -> element schema (for nested `each`).
...
impl Indexer {
    ...
    fn stmt(&mut self, in_def: bool, syms: &mut Vec<Sym>, top: bool) {
        ...
        if top && t.kind == TokKind::Id && t.text == "icon" && self.icon_decl_ahead() {
            ...
            self.ix.icons.insert(
                name.text.clone(),
                (name.line, name.col, name.end, detail.clone()),
            );
        ...
        if t.kind == TokKind::Id && t.text == "def" && self.at_k(TokKind::Id, 1) {
            ...
            self.ix.defs.insert(
                name.text.clone(),
                (name.line, name.col, name.end, sig.clone()),
            );
    ...
    /// Consume the parameter list, returning its signature and List-valued fields.
    fn def_sig(&mut self, name: &LTok) -> (String, Vec<(String, String)>) {
        ...
        let list_fields = normalized
            ...
            .collect();
        (
            format!("def {}({})", name.text, normalized.join(", ")),
            list_fields,
        )
    }
...
/// Index definitions, icons, list fields, tokens, params, symbols, colors, and blocks.
