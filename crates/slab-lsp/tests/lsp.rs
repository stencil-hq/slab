...
const UNI_LINE: &str = "  text#emoji   \"ok \u{1F44D} team \
                        \u{1F469}\u{200D}\u{1F469}\u{200D}\u{1F467} done\" size=14 color=color.ink";
...
fn server_with(text: &str) -> (Server, Vec<Value>) {
	...
	let notes = srv.handle(&json!({"jsonrpc": "2.0", "method": "textDocument/didOpen", "params": {
...
fn test_did_change_incremental_updates_diagnostics() {
	...
	let notes =
		srv.handle(&json!({"jsonrpc": "2.0", "method": "textDocument/didChange", "params": {
...
fn test_document_symbols_include_def_and_ids() {
	let (mut srv, _) = server();
	let syms = req(&mut srv, "textDocument/documentSymbol", json!({"textDocument": {"uri": URI}}));
...
fn test_document_color_hex_and_oklch() {
	let (mut srv, _) = server();
	let colors = req(&mut srv, "textDocument/documentColor", json!({"textDocument": {"uri": URI}}));
	...
	let starts: Vec<(i64, i64)> = colors
		...
		.collect();
	assert!(starts.contains(&(1, line1.find("#0e1116").unwrap() as i64)), "{starts:?}");
	// color function gets a swatch too
	assert!(starts.contains(&(1, line1.find("oklch").unwrap() as i64)), "{starts:?}");
	for c in colors {
...
fn test_formatting_full_document_edit() {
	...
	assert_eq!(edits[0]["newText"], json!("col w=fill {\n  text \"hi\" size=12\n}\n"));
...
"#;
	...
	let notes =
		server.handle(&json!({"jsonrpc": "2.0", "method": "textDocument/didOpen", "params": {
			 "textDocument": {
				  "uri": root_uri,
				  "languageId": "slab",
				  "version": 1,
	...
	let symbols =
		req(&mut server, "textDocument/documentSymbol", json!({"textDocument": {"uri": module_uri}}));
	assert!(
	...
	let notes =
		server.handle(&json!({"jsonrpc": "2.0", "method": "textDocument/didChange", "params": {
