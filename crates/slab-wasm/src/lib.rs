...
use slab_compile::{
	Options, compile, compile_with_exports, expand,
	react::generate as gen_react_files,
	render::{RenderKind, RenderOpts, render as render_slir},
	rustgen::generate as gen_rust_src,
	wc::{WcFile, WcOptions, generate as gen_wc_files},
};
...
/// Serialize diagnostics as JSON: `[{level, code, msg, line, remedy,
/// formatted}]`. `formatted` is `Diag::format(file_name)` so CLIs print
/// byte-identical text.
fn diags_json(diags: &Diagnostics, file: &str) -> String {
	...
	for (i, d) in diags.0.iter().enumerate() {
		...
		let entry = serde_json::json!({
			 ...
			 "formatted": serde_json::from_str::<serde_json::Value>(&formatted).unwrap_or(serde_json::Value::String(String::new())),
		});
...
mod tests {
	...
	fn virtual_import_apis_compile_a_complete_source_map() {
		...
		assert!(!diagnostics.contains("\"level\":\"error\""), "{diagnostics}");
