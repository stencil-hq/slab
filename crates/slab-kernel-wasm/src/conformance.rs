...
pub fn cells_text(instance: &mut Instance, time_ms: f64) -> String {
...
pub fn cells_attrs(instance: &mut Instance, time_ms: f64) -> String {
	let solved = frame::inst_frame(instance, time_ms);
	let grid = cells::cells_from_frame(instance.doc(), &solved, solved.width, solved.height);
...
pub fn caps_report(instance: &mut Instance, time_ms: f64, client: u32) -> Result<String, String> {
	...
	lines.extend(capability::chart_lines(instance.doc(), &solved, client_index));
...
pub fn selftest_counts_json(instance: &mut Instance, time_ms: f64) -> String {
	let solved = frame::inst_frame(instance, time_ms);
