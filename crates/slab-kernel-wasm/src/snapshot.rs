...
struct Statics<'a> {
	...
	font_ascent:              &'a [i32],
	font_descent:             &'a [i32],
	font_default_adv:         &'a [u32],
	font_underline_position:  &'a [i32],
	font_underline_thickness: &'a [i32],
	font_cmap_off:            &'a [i32],
	font_cmap_len:            &'a [i32],
	font_cmap_cp:             &'a [u32],
	font_cmap_gid:            &'a [u32],
...
pub fn statics_json(instance: &Instance) -> String {
	...
	let signals = document
		...
		.map(|(&name, &trigger)| SignalDef { name: string_at(document, name), trigger })
		.collect();
	let holes = document
		.hole_name
		.iter()
	...
	let statics = Statics {
		...
		font_default_adv: &document.font_default_adv,
		font_underline_position: &document.font_underline_position,
		font_underline_thickness: &document.font_underline_thickness,
		font_cmap_off: &document.font_cmap_off,
		font_cmap_len: &document.font_cmap_len,
		font_cmap_cp: &document.font_cmap_cp,
		font_cmap_gid: &document.font_cmap_gid,
		font_adv: &document.font_adv,
...
pub fn effects_json(effects: &Effects) -> String {
	...
	let scrolls = effects
		.scrolls
		.iter()
		.map(|scroll| ScrollChangeSnapshot { key: &scroll.key, axis: scroll.axis, off: scroll.off })
		.collect();
...
pub fn holes_json(holes: &[HoleRect]) -> String {
...
pub fn lifts_json(lifts: &[motion::Lift]) -> String {
...
pub fn chain_json(instance: &Instance, scene_index: i32) -> String {
	let mut chain = Vec::new();
	scene::chain(&instance.sc, scene_index, &mut chain);
	to_json(&chain)
}

pub fn hit_contains(instance: &Instance, scene_index: i32, x: f64, y: f64) -> bool {
...
const fn optional_bool(code: u32) -> Option<bool> {
	match code {
		1 => Some(false),
		2 => Some(true),
		_ => None,
...
const fn checked_snapshot(code: u32) -> Option<CheckedSnapshot> {
	match code {
...
const fn live_snapshot(code: u32) -> Option<&'static str> {
	match code {
		1 => Some("off"),
		2 => Some("polite"),
		3 => Some("assertive"),
