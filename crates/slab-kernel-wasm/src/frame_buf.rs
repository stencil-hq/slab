...
mod tests {
	...
	fn encodes_dimensions_signed_indices_and_operation_tags() {
		...
		frame.ops = vec![
			...
			FrameOp::GroupPop,
			FrameOp::RotatePop,
			FrameOp::ScalePush(OpScale { cx: 4.0, cy: 5.0, sx: 2.0, sy: 3.0 }),
			FrameOp::ScalePop,
		];
		frame.strings = vec!["hello".to_owned()];
		frame.uncovered = vec![0, 5];
		frame.paths_rt = vec![RtPath { verbs: vec![0, 1], coords: vec![1.0, 2.0, 3.0, 4.0] }];
		...
		assert_eq!(encoded.f64s, [
			320.0, 180.0, 10.0, 20.0, 30.0, 16.0, 0.5, 0.75, 0.0, 0.0, 0.0, 0.0, 0.5, 2.0, 1.0, 2.0,
			30.0, 40.0, 4.0, 5.0, 2.0, 3.0,
		]);
