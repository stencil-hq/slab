...
fn roundtrip_byte_identity_over_corpus() {
    ...
    for path in cases {
        ...
        assert_eq!(
            bytes,
            bytes2,
            "{}: write->read->write is not byte-identical",
            path.display()
        );
...
"#;
    ...
    assert_eq!(
        (
            slir.lists[1].param,
            slir.lists[1].field_off,
            slir.lists[1].field_len
        ),
        (0, 2, 2)
    );
...
"#;
    ...
    assert!(
        frame.ops.iter().any(
            |op| matches!(op, slab_kernel::flatten::FrameOp::PathDraw(path) if path.path >= 0)
        )
    );
    assert!(
        frame
...
"#;
    ...
    for icon in &slir.icons {
        assert_ne!(
            slir.nodes.flags[icon.node as usize] & slab_slir::flags::DETACHED,
            0
        );
    }
