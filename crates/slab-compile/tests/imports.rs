...
fn root_definition_shadows_an_imported_definition() {
    let options = virtual_options([(
        "module.slab",
        "def Card() { text \"IMPORTED_SENTINEL\" }\n".to_string(),
    )]);
...
fn first_cross_file_parameter_declaration_wins() {
    ...
    assert_eq!(
        slir.str_at(slir.avals[param.default as usize].lo()),
        "MODULE_DEFAULT"
    );
}
...
"#
    ...
    apply_sets(
        &mut instance,
        &[("panel.open".to_string(), "false".to_string())],
    )
