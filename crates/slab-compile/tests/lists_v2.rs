...
"#;
	...
	assert_eq!(nested.tag, aval::LIST_DEFAULT);
	assert_eq!(nested.hi(), 0, "leaf children normalize to an empty list run");
	...
	let eaches: Vec<usize> = slir
		...
		.collect();
	assert_eq!(eaches.len(), 2, "one root each and one symbolic recursive edge");
...
"#;
	...
	for attr in
		[attrs::CONTENT, attrs::COLOR, attrs::SIZE, attrs::WEIGHT, attrs::FAMILY, attrs::TRACKING]
	{
		let value = slir
...
"#;
	...
	assert!(frame::inst_set_list_key(&mut instance, 0, "0.chips", 0, "inner"));
