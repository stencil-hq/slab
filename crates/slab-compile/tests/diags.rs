...
";
	...
	for (name, trigger) in [
		...
		assert!(
			slir
				.signals
				.iter()
				.any(|&(name_ref, _, candidate)| slir.str_at(name_ref) == name && candidate == trigger)
		);
	}
	for name in ["pressed", "started"] {
		...
		assert_ne!(slir.nodes.flags[node as usize] & slab_slir::flags::FOCUSABLE, 0, "{name}");
	}
	for name in ["menu", "twice", "dropped", "hovered", "released"] {
		...
		assert_eq!(slir.nodes.flags[node as usize] & slab_slir::flags::FOCUSABLE, 0, "{name}");
	}
	let drag_node = slir
		.signals
		.iter()
		...
		.expect("drag node");
	assert_ne!(slir.nodes.flags[drag_node as usize] & slab_slir::flags::DRAG_GHOST, 0);
}
...
fn drag_companions_require_drag_binding() {
	let (slir, diagnostics) =
		compile("box drag-update=updated drag-end=ended drag-ghost\n", &Options {
			embed_assets: false,
			..Default::default()
		});
	...
	for message in ["`drag-update=` requires", "`drag-end=` requires", "`drag-ghost` requires"] {
		assert!(
			diagnostics
				.0
				.iter()
...
fn escape_blur_requires_an_editable_text_node() {
	...
	let text_node = slir
		...
		.expect("text node");
	assert_eq!(slir.nodes.flags[box_node] & slab_slir::flags::ESCAPE_BLUR, 0);
	assert_ne!(slir.nodes.flags[text_node] & slab_slir::flags::ESCAPE_BLUR, 0);
...
fn scrollbar_rejects_unknown_mode() {
	assert_has("col scroll scrollbar=\"sometimes\" { }\n", "ref", Level::Error, 1);
}
...
fn activation_keys_imply_focusable() {
	let (slir, diags) = compile("col keys=Escape,F2 act=cancel { }\n", &Options {
		embed_assets: false,
		..Default::default()
	});
	assert!(!diags.has_errors(), "{:?}", diags.0);
	let slir = slir.unwrap();
	assert_ne!(slir.nodes.flags[0] & slab_slir::flags::FOCUSABLE, 0, "keys implies focusable");
	let value = slir
		.node_attrs(0)
		.iter()
...
fn mapped_activation_keys_register_distinct_signals() {
	let (slir, diags) = compile("col keys=Escape:clear,F2:rename { }\n", &Options {
		embed_assets: false,
		..Default::default()
	});
	assert!(!diags.has_errors(), "{:?}", diags.0);
...
fn mapped_activation_keys_reject_act_ambiguity() {
	assert_has("col keys=Escape:clear act=activate { }\n", "keys", Level::Error, 1);
}
...
fn field_sync_host_opt_out_suppresses_warning() {
	...
	let (_, diags) = compile(src, &Options::default());
	assert!(diags.0.iter().all(|diag| diag.code != "field-sync"), "{:#?}", diags.0);
}
...
fn divider_requires_a_middle_row_or_col_position() {
	assert_has("row {\n  divider w=6\n  rect w=fill\n}\n", "divider-ctx", Level::Error, 2);
	assert_has("row {\n  rect w=fill\n  divider w=6\n}\n", "divider-ctx", Level::Error, 3);
	assert_has(
		"stack {\n  rect w=40\n  divider w=6\n  rect w=40\n}\n",
		"divider-ctx",
		Level::Error,
...
fn divider_accepts_fixed_percentage_hug_and_fill_footprints() {
	for extent in ["6", "10%", "hug", "fill"] {
		...
		let (_, diags) = compile(&source, &Options { embed_assets: false, ..Default::default() });
...
fn icon_declarations_reject_duplicate_names_and_non_path_bodies() {
	assert_has("icon mark {\n  rect w=4 h=4\n}\n", "icon-body", Level::Error, 1);
	assert_has(
...
"##;
	let (slir, diags) = compile(source, &Options { embed_assets: false, ..Default::default() });
	assert!(!diags.has_errors(), "{:?}", diags.0);
...
fn accessibility_semantics_reject_invalid_domains_ranges_and_keys() {
	...
	assert_has("col value-min=10 value-now=5 value-max=20\n", "a11y-range", Level::Error, 1);
	assert_has("col controls=\"#missing\"\n", "a11y-key", Level::Error, 1);
	assert_has("col pos-in-set=3 set-size=2\n", "a11y-range", Level::Error, 1);
}
...
"#;
	...
	assert!(matches!(ghost.ops.last(), Some(slab_kernel::flatten::FrameOp::GroupPop)));
	assert!(
		ghost
			.ops
			.iter()
...
";
	...
	slab_kernel::frame::inst_dispatch(
		&mut instance,
		&event(slab_kernel::dispatch::E_POINTER_MOVE, down_x + 20.0, down_y, 20.0),
	);
	let ghost = slab_kernel::frame::inst_frame(&mut instance, 1.0);
...
"#;
	...
	assert!(diagnostic.msg.contains("in export TodoItem"), "{diagnostic:?}");
	assert!(
		diagnostic
			.format("todo.slab")
...
fn field_sync_wording_names_export_prop_and_declaration_site() {
	let source = "def TaskRow(title=\"\", editing=false) export {\n  row { text title { when \
	              editing { field=title_edit; submit=commit } } }\n}\nparams { tasks list(TaskRow) \
...
fn labeled_or_text_bearing_focusables_do_not_warn_a11y_name() {
	for source in [
		...
		assert!(
			ds.iter().all(|(code, ..)| code != "a11y-name"),
...
fn binder_outside_inert_subtree_does_not_warn() {
	let ds = diags_of("col w=fill {\n  box press=pressed w=8 h=8\n}\n");
	assert!(ds.iter().all(|(code, ..)| code != "inert-binder"), "{ds:?}");
}
...
fn bare_flag_in_when_patch_body_parses_before_attrs() {
	...
	let ds = diags_of(
		"params { done bool = false }\ncol w=fill {\n  text \"t\" { when done { strike \
		 color=#888888 } }\n}\n",
	);
	assert!(
		ds.iter()
			.all(|(code, level, _)| { code != "parse" && !(code == "ref" && *level == Level::Error) }),
