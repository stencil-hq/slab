...
"#,
    ...
    assert_eq!(
        instance.sc.y[h1_scene], -10.0,
        "next header pushes the first away"
    );
    assert_eq!(
        instance.sc.y[h2_scene], 10.0,
        "next header approaches the start edge"
    );
...
fn sticky_rejects_roots_and_cross_only_parents() {
    for source in ["col sticky { }\n", "col scroll=cross { rect sticky }\n"] {
        ...
        assert!(slir.is_none());
        assert!(
            diagnostics.0.iter().any(|diag| diag.code == "sticky-ctx"),
            "{:#?}",
            diagnostics.0
        );
    }
