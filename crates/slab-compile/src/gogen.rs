...
//! `list.set_field`, and `DecodeSignals` turns raw effects into typed signals.
//! The kernel remains the only owner of layout, hit testing, focus, and
//! editing; the generated code never reimplements any of it.
//!
...
fn emit_list_types(out: &mut String, slir: &Slir, schema_row: usize, param_name: &str) {
    ...
    for &row in &order {
        ...
        for field_ix in schema.field_off..schema.field_off + schema.field_len {
            ...
            let _ = writeln!(
                out,
                "\t// {member} is schema field `{field_name}` ({note})."
            );
        ...
        let _ = writeln!(
            ...
            "func validate{base}Items(items []{item_ty}) error {{\n\
             ...
             \t\treturn fmt.Errorf(\"{item_ty} list has %d items, more than the %d the protocol allows\", len(items), maxListLen)\n\
             \t}}\n\
             ...
             \t\t\treturn fmt.Errorf(\"{item_ty} %d: duplicate key %q\", index, key)\n\
             \t\t}}\n\
             \t\tkeys[key] = struct{{}}{{}}"
        );
        for field_ix in schema.field_off..schema.field_off + schema.field_len {
            ...
            if field.ty == 5 {
                ...
                let members: Vec<String> = (field.enum_off..field.enum_off + field.enum_len)
                    .map(|ix| {
                        format!(
                            "{enum_ty}{}",
                            pascal(slir.str_at(slir.list_enum_syms[ix as usize]))
                        )
                    })
                    .collect();
                ...
                let _ = writeln!(
                    out,
                    "\t\tdefault:\n\
                     \t\t\treturn fmt.Errorf(\"{item_ty} %d: field `{field_name}` has unknown enum member %q\", index, string(item.{member}))\n\
                     \t\t}}"
                );
            } else if field.ty == 6 {
                let child_ty = list_type_name(&names, slir, field.sub as usize - 1);
...
fn emit_list_setters(out: &mut String, slir: &Slir, schema_row: usize, param_name: &str) {
    ...
    for &row in &order {
        ...
        let _ = writeln!(
            out,
            "func (d *Doc) set{base}Path(ctx context.Context, path string, items, previous []{item_ty}) error {{\n\
             \tif err := d.setListLen(ctx, {wire_param}, path, len(items)); err != nil {{\n\t\treturn err\n\t}}\n\
             \tfor index, item := range items {{\n\
             \t\tvar prior *{item_ty}\n\
             ...
             \t\t\tkey := item.Key\n\
             \t\t\tif key == \"\" {{\n\t\t\t\tkey = strconv.Itoa(index)\n\t\t\t}}\n\
             \t\t\tif err := d.setListKey(ctx, {wire_param}, path, index, key); err != nil {{\n\t\t\t\treturn err\n\t\t\t}}\n\
             \t\t}}"
        );
        for field_ix in schema.field_off..schema.field_off + schema.field_len {
            let field = &slir.list_fields[field_ix as usize];
            ...
            if field.ty == 6 {
                ...
                let _ = writeln!(
                    ...
                    "\t\tchildPath := strconv.Itoa(index) + {}\n\
                     ...
                     \t\tvar prior{member} []{child_ty}\n\
                     \t\tif prior != nil {{\n\t\t\tprior{member} = prior.{member}\n\t\t}}\n\
                     \t\tif err := d.set{child_base}Path(ctx, childPath, item.{member}, prior{member}); err != nil {{\n\t\t\treturn err\n\t\t}}",
            ...
            let _ = writeln!(
                ...
                "\t\tif prior == nil || prior.{member} != item.{member} {{\n\
                 \t\t\tif err := d.setListField(ctx, {wire_param}, path, index, {}, {}, {}); err != nil {{\n\t\t\t\treturn err\n\t\t\t}}\n\
                 \t\t}}",
    ...
    let _ = writeln!(
        ...
        "func (d *Doc) Set{setter}(ctx context.Context, items []{root_ty}) error {{\n\
         \tif d.{cache}Valid && equal{root_base}Items(d.{cache}, items) {{\n\t\treturn nil\n\t}}\n\
         \tif err := validate{root_base}Items(items); err != nil {{\n\t\treturn err\n\t}}\n\
         \tprevious := d.{cache}\n\
...
fn emit_module(slir: &Slir, bytes: &[u8], src_name: &str, package: &str) -> String {
    ...
    for (name, key) in crate::wc::static_scene_keys(slir) {
        ...
        let _ = writeln!(
            o,
            "// {konst} is the canonical scene key of the authored `#{name}` node."
        );
    ...
    for (param_ix, p) in slir.params.iter().enumerate() {
        ...
        let _ = writeln!(
            o,
            "\t// {cache} is the last value applied by [Doc.Set{}].",
            pascal(name)
        );
        let _ = writeln!(o, "\t{cache} []{root_ty}");
        let _ = writeln!(
            o,
            "\t// {cache}Valid reports whether {cache} holds an applied value."
        );
...
"#;
        ...
        assert!(module.contains("case SignalPressed, SignalMoved, SignalReleased, SignalResized, SignalTwice, SignalStarted, SignalUpdated, SignalEnded:"));
        assert!(
...
"#;
        ...
        assert!(
            module.contains("func (d *Doc) SetTitle(ctx context.Context, value string) error {")
        );
        ...
        assert!(
            module.contains("func New(ctx context.Context, sess *slab.Session) (*Doc, error) {")
        );
