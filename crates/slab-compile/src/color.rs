...
fn srgb(x: f64) -> f64 {
	if x <= 0.003_130_8 {
		...
		1.055f64.mul_add(x.powf(1.0 / 2.4), -0.055)
	}
}

fn oklab_to_rgb(l: f64, a: f64, b: f64) -> [u8; 3] {
	let l_ = 0.215_803_757_3f64.mul_add(b, 0.396_337_777_4f64.mul_add(a, l)).powi(3);
	let m_ = 0.063_854_172_8f64.mul_add(-b, 0.105_561_345_8f64.mul_add(-a, l)).powi(3);
	let s_ = 1.291_485_548_0f64.mul_add(-b, 0.089_484_177_5f64.mul_add(-a, l)).powi(3);
	let r = 0.230_969_929_2f64.mul_add(s_, 3.307_711_591_3f64.mul_add(-m_, 4.076_741_662_1 * l_));
	let g = 0.341_319_396_5f64.mul_add(-s_, 2.609_757_401_1f64.mul_add(m_, -1.268_438_004_6 * l_));
	let bl = 1.707_614_701_0f64.mul_add(s_, 0.703_418_614_7f64.mul_add(-m_, -0.004_196_086_3 * l_));
...
fn parse_stops(parts: &[String]) -> Option<Vec<Stop>> {
	...
	Some(
		raw.into_iter()
			.map(|(c, o)| Stop { offset: o.unwrap().clamp(0.0, 1.0), rgba: c })
			.collect(),
	)
}
...
pub fn parse_paint(s: &str) -> Option<Paint> {
	...
	Some(match kind {
		"linear" => Paint::Linear { angle: angle.rem_euclid(360.0), stops },
		"conic" => Paint::Conic { from: angle.rem_euclid(360.0), stops },
		_ => Paint::Radial { stops },
	})
}

/// rgba8 word: byte 0 = r, byte 1 = g, byte 2 = b, byte 3 = a.
pub const fn rgba_word(c: Rgba) -> u32 {
	u32::from_le_bytes(c)
}
