...
fn emit_module(docs: &[DocSpec], separate_ir: bool) -> String {
	...
	for d in docs {
		let _ = writeln!(
			m,
			"\n/** Template-relative scene keys per `each` list; join with `itemKey(each, item, \
			 rel)`. */"
		);
	...
	for (name, has_text) in merged_signals(docs) {
		let detail = if has_text {
			"{ text: '', item: '', meta: { x: -1, y: -1, dx: 0, dy: 0, drag_dx: 0, drag_dy: 0, mods: \
			 0, button: 0, clicks: 0, key: '', src_key: '', src_item: '', cancelled: false, dropped: \
			 false } }"
		} else {
			"{ item: '', meta: { x: -1, y: -1, dx: 0, dy: 0, drag_dx: 0, drag_dy: 0, mods: 0, button: \
			 0, clicks: 0, key: '', src_key: '', src_item: '', cancelled: false, dropped: false } }"
		};
...
fn emit_dts(docs: &[DocSpec]) -> String {
	...
	m.push_str("// GENERATED by `slab gen wc` — do not edit.\n");
	m.push_str(
		"\n/** Join one `each` item into a full canonical scene key.\n\x20* `itemKey(each, item)` \
		 addresses the item root; `itemKey(each, item, rel)`\n\x20* appends a template-relative \
		 key. `item` is escaped per the canonical\n\x20* scene-key grammar (`%` → `%25`, `/` → \
		 `%2F`, `~` → `%7E`). */\nexport declare function itemKey(each: string, item: string | \
		 number, rel?: string): string;\n",
	);
	m.push_str(
	...
	m.push_str(
		"\n/** One retained scene entry returned by `sceneSnapshot()`. */\nexport interface \
		 ...
		 null;\n\x20  disabled: boolean;\n\x20  focused: boolean;\n\x20  /** Whether the node \
		 carries an ACTIVE `field=` binder this frame. */\n\x20  editable: boolean;\n\x20  /** \
	...
	for d in docs {
		...
		m.push_str(
			"   addEventListener(type: 'slab-diagnostics', listener: (event: \
			 CustomEvent<SlabDiagnosticsDetail>) => void, options?: boolean | \
		...
		m.push_str(
			"   imgRegister(name: string, width: number, height: number, format: number, bytes: \
			 Uint8Array): number;\n",
		);
		...
		m.push_str(
			"   imgInfo(image: number): readonly [width: number, height: number, format: number, \
		...
		for (n, ty) in &d.params {
			...
			if d.lists.iter().any(|list| list.name == *n) {
				...
				let _ = writeln!(m, "   get '{n}'(): {item_name}[];\n   set '{n}'(v: {item_name}[]);");
			} else {
				let t = ts_type(ty);
		...
		if !d.lists.is_empty() {
			...
			m.push_str("   /** Read the last root or nested list accepted at its runtime path. */\n");
