...
fn detail_type(has_text: bool) -> &'static str {
    if has_text {
        "TextSignalDetail"
    } else {
        "SignalDetail"
...
fn emit_element_interface(m: &mut String, doc: &DocSpec, comp: &str) {
    let _ = write!(
        m,
        "\n/** Property surface of `<{}>` the wrapper assigns through. */\n\
         interface {comp}Element extends HTMLElement {{\n",
        doc.tag
    );
...
fn emit_component(m: &mut String, doc: &DocSpec, comp: &str) {
    let _ = write!(
        m,
        "\n/** Typed React wrapper for `<{}>`. */\n\
         export function {comp}(props: {comp}Props): React.JSX.Element {{\n\
         \x20  const ref = React.useRef<{comp}Element | null>(null);\n",
        doc.tag
    );
    ...
    for (signal, has_text) in &doc.signals {
        ...
        let _ = write!(
            ...
            "   React.useEffect(() => {{\n\
             ...
             \x20     const handler = props.{prop};\n\
             \x20     if (!el || !handler) {{\n\
             \x20        return undefined;\n\
             \x20     }}\n\
             \x20     const listener = (event: Event) =>\n\
             \x20        handler((event as CustomEvent<{detail}>).detail);\n\
             \x20     el.addEventListener('{signal}', listener);\n\
...
fn emit_tsx(docs: &[DocSpec], stem: &str) -> String {
    ...
    if docs.iter().any(|doc| !doc.signals.is_empty()) {
...
"#;
    ...
    fn wrapper_declares_typed_props_handlers_and_property_assignment() {
        ...
        assert!(tsx.contains("export function Settings(props: SettingsProps): React.JSX.Element"));
    ...
    fn wrapper_generation_is_deterministic_and_keeps_wc_files() {
        ...
        for expected in [
            ...
            assert!(
                first.iter().any(|file| file.name == expected),
                "missing {expected}"
            );
        }
    }
