...
fn emit_element_interface(m: &mut String, doc: &DocSpec, comp: &str) {
    let _ = write!(
        ...
        "\n/** Imperative web-component surface for `<{}>`. */\n\
         export interface {comp}Element extends HTMLElement {{\n",
        doc.tag
    );
...
fn emit_component(m: &mut String, doc: &DocSpec, comp: &str) {
    let _ = write!(
        ...
        "\n/** Typed React wrapper for `<{}>`; its ref exposes the element's imperative API. */\n\
         export const {comp} = React.forwardRef<{comp}Element, {comp}Props>(function {comp}(props, forwardedRef): React.JSX.Element {{\n\
         \x20  const ref = React.useRef<{comp}Element | null>(null);\n\
         \x20  React.useImperativeHandle(forwardedRef, () => ref.current as {comp}Element, []);\n",
        doc.tag
    );
