...
fn emit_module(slir: &Slir, bytes: &[u8], src_name: &str) -> String {
    ...
    for (i, p) in slir.params.iter().enumerate() {
        let _ = writeln!(
            o,
            "pub const PARAM_{}: u32 = {i};",
            snake(slir.str_at(p.name)).to_uppercase()
        );
    }
    for (i, &(name, _node)) in slir.holes.iter().enumerate() {
        let _ = writeln!(
            o,
            "pub const HOLE_{}: u32 = {i};",
            snake(slir.str_at(name)).to_uppercase()
        );
    }
    ...
    for (name, _) in &sigs {
        ...
        let strref = slir
            .signals
            .iter()
            .find(|&&(n, _, _)| slir.str_at(n) == name)
            .map(|&(n, _, _)| n)
            .unwrap_or(0);
        let _ = writeln!(
            o,
            "const SIG_{}: u32 = {strref};",
    ...
    o.push_str(
        "\n/// Metadata captured for every emitted signal.\n\
         ...
         \x20   /// Document-space pointer x, or `-1.0` for keyboard origin.\n\
         \x20   pub x: f64,\n\
         \x20   /// Document-space pointer y, or `-1.0` for keyboard origin.\n\
         \x20   pub y: f64,\n\
         \x20   /// Horizontal delta carried by the originating event.\n\
         \x20   pub dx: f64,\n\
         \x20   /// Vertical delta carried by the originating event.\n\
         \x20   pub dy: f64,\n\
         \x20   /// Horizontal drag displacement from the pointer-down origin.\n\
         \x20   pub drag_dx: f64,\n\
         \x20   /// Vertical drag displacement from the pointer-down origin.\n\
         \x20   pub drag_dy: f64,\n\
         \x20   /// Modifier bitset active at emission.\n\
         \x20   pub mods: u32,\n\
         \x20   /// Pointer button code.\n\
         \x20   pub button: u32,\n\
         \x20   /// Host-computed click count.\n\
         \x20   pub clicks: u32,\n\
         \x20   /// Full key path of the signal-emitting node.\n\
         \x20   pub key: String,\n\
         \x20   /// Full drag-source key for Drop, otherwise empty.\n\
         \x20   pub src_key: String,\n\
         \x20   /// Innermost drag-source item key for Drop, otherwise empty.\n\
         \x20   pub src_item: String,\n\
         \x20   /// Whether DragEnd represents abnormal termination.\n\
         ...
         \x20           drag_dy: meta.drag_dy,\n\
         \x20           mods: meta.mods,\n\
         \x20           button: meta.button,\n\
         \x20           clicks: meta.clicks,\n\
         \x20           key: meta.key.clone(),\n\
            ...
            for field_ix in schema.field_off..schema.field_off + schema.field_len {
                ...
                let (rust_ty, type_note) = match field.ty {
                    ...
                    6 if field.sub != 0 => (
                        format!(
                            "Vec<{}>",
                            list_type_name(&names, slir, field.sub as usize - 1)
                        ),
    ...
    let _ = writeln!(
        ...
        "/// The document instance, driven through `slab-kernel`.\n\
         ...
         \x20   pub fn new() -> Doc {{\n\
         \x20       let (doc, imgs) = slab_slir::decode_doc(SLIR).expect(\"embedded SLIR\");\n\
         ...
         \x20   /// Env for the gpu client (client code 1); portrait derives from vw < vh.\n\
         \x20   pub fn set_env(&mut self, vw: f64, vh: f64, dark: bool, coarse: bool) {{\n\
         \x20       kframe::inst_set_env(&mut self.inst, vw, vh, 1, dark, coarse);\n\
         \x20   }}\n\n\
         \x20   /// Select a compiler-declared theme; an empty name restores authored values.\n\
         ...
         \x20       kframe::inst_theme(&self.inst)\n\
         \x20   }}\n\n\
         \x20   /// Solve and flatten the document at `t_ms` milliseconds.\n\
         \x20   pub fn frame(&mut self, t_ms: f64) -> Frame {{\n\
         \x20       kframe::inst_frame(&mut self.inst, t_ms)\n\
         \x20   }}\n\n\
         \x20   /// Return the current host-content hole rectangles.\n\
         \x20   pub fn holes(&mut self) -> Vec<HoleRect> {{\n\
         \x20       kframe::inst_holes(&mut self.inst)\n\
         \x20   }}\n\n\
         \x20   /// Set a keyed scroll node's offset on axis 0 (main) or 1 (cross).\n\
         \x20   pub fn set_scroll(&mut self, key: &str, axis: u32, off: f64) -> bool {{\n\
         \x20       kframe::inst_set_scroll(&mut self.inst, key, axis, off)\n\
         \x20   }}\n\n\
         \x20   /// Read a keyed scroll node's offset on axis 0 (main) or 1 (cross).\n\
         \x20   pub fn get_scroll(&self, key: &str, axis: u32) -> f64 {{\n\
         \x20       kframe::inst_get_scroll(&self.inst, key, axis)\n\
         \x20   }}\n\n\
         \x20   /// Register or replace a named runtime image and return its unified image index.\n\
         \x20   pub fn img_register(\n\
         \x20       &mut self,\n\
         \x20       name: &str,\n\
         \x20       w: u32,\n\
         \x20       h: u32,\n\
         ...
         \x20       data: &[u8],\n\
         \x20   ) -> i32 {{\n\
         \x20       kframe::inst_img_register(&mut self.inst, name, w, h, format, data)\n\
         \x20   }}\n\n\
         \x20   /// Unregister a named runtime image while preserving its unified image index.\n\
         \x20   pub fn img_unregister(&mut self, name: &str) -> bool {{\n\
         \x20       kframe::inst_img_unregister(&mut self.inst, name)\n\
         \x20   }}\n\n\
         \x20   /// Scroll ancestors minimally to reveal a keyed node with the requested margin.\n\
         \x20   pub fn reveal(&mut self, key: &str, margin: f64) -> bool {{\n\
         \x20       kframe::inst_reveal(&mut self.inst, key, margin)\n\
         \x20   }}\n\n\
         \x20   /// Reveal an item in a virtual list using start, center, end, or nearest alignment.\n\
         \x20   pub fn reveal_item(&mut self, each_key: &str, index: i32, align: u32) -> bool {{\n\
         \x20       kframe::inst_reveal_item(&mut self.inst, each_key, index, align)\n\
         \x20   }}\n\n\
         \x20   /// Return a virtual list's materialized half-open item window.\n\
         \x20   pub fn each_window(&self, each_key: &str) -> (i32, i32) {{\n\
         \x20       kframe::inst_each_window(&self.inst, each_key)\n\
         \x20   }}\n\n\
         \x20   /// Set the extent overlay controlled by a keyed divider.\n\
         \x20   pub fn set_divider(&mut self, key: &str, extent: f64) -> bool {{\n\
         \x20       kframe::inst_set_divider(&mut self.inst, key, extent)\n\
         \x20   }}\n\n\
         \x20   /// Read a keyed divider's extent overlay, or `-1.0` when unknown or unset.\n\
         \x20   pub fn get_divider(&self, key: &str) -> f64 {{\n\
         \x20       kframe::inst_get_divider(&self.inst, key)\n\
         \x20   }}\n\n\
         \x20   /// Dispatch an input event and return its raw and typed effects.\n\
         \x20   pub fn dispatch(&mut self, ev: &Event) -> (Effects, Vec<Signal>) {{\n\
         \x20       let eff = kframe::inst_dispatch(&mut self.inst, ev);\n\
         \x20       let sigs = self.decode_signals(&eff);\n\
         \x20       (eff, sigs)\n\
         \x20   }}\n\n\
         \x20   /// Drain solve-time signals queued by the most recent frame.\n\
         \x20   pub fn take_signals(&mut self) -> (Effects, Vec<Signal>) {{\n\
         \x20       let eff = kframe::inst_take_signals(&mut self.inst);\n\
         \x20       let sigs = self.decode_signals(&eff);\n\
         \x20       (eff, sigs)\n\
         \x20   }}\n\n\
         \x20   /// Decode Effects signal refs against the document's STRS pool.\n\
         \x20   pub fn decode_signals(&self, eff: &Effects) -> Vec<Signal> {{\n\
    ...
    for (name, has_text) in &sigs {
        ...
        if *has_text {
            let _ = writeln!(
                o,
                "                {konst} => out.push(Signal::{} {{ text: eff.sig_text[i].clone(), item: eff.sig_item[i].clone(), meta: SignalMeta::from(&eff.sig_meta[i]) }}),",
                pascal(name)
            );
        } else {
            let _ = writeln!(
                o,
                "                {konst} => out.push(Signal::{} {{ item: eff.sig_item[i].clone(), meta: SignalMeta::from(&eff.sig_meta[i]) }}),",
                pascal(name)
            );
        }
    }
    ...
    for (i, p) in slir.params.iter().enumerate() {
        ...
        let _ = writeln!(
            ...
            "\n    /// Set param `{name}` ({doc_note}); false = rejected by the kernel.\n\
             \x20   pub fn set_{method}(&mut self, {sig}) -> bool {{\n\
             ...
             \x20       }};\n\
             \x20       {fill}\n\
             \x20       kframe::inst_set_param(&mut self.inst, {i}, &pv)\n\
             \x20   }}",
            ty = p.ty
        );
    }
