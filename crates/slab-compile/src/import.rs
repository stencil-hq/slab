...
impl Loader<'_> {
	...
	fn load(&mut self, importer: Option<&str>, import: &AImport) {
		...
		self
			.units
			.push(Unit { file: Some(key), abs, doc: document });
	}
...
pub fn closure(root_src: &str, opts: &Options, diags: &mut Diagnostics) -> Vec<Unit> {
	...
	let mut loader =
		Loader { opts, diags, units: Vec::new(), visited: HashSet::new(), stack: Vec::new() };
	for import in &root.imports {
		loader.load(None, import);
	}
	loader.units.push(Unit { file: None, abs: None, doc: root });
	loader.units
}
...
mod tests {
	...
	fn normalizes_imports_relative_to_the_importer() {
		...
		assert_eq!(normalize(Some("ui/panels/view.slab"), "../theme.slab"), "ui/theme.slab");
