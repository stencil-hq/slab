...
fn rgba8(v: u32, opacity: f64) -> Color {
	...
	Color::from_rgba8(r, g, b, (a as f64 * opacity).round().clamp(0.0, 255.0) as u8)
}

fn base_paint() -> Paint<'static> {
	Paint { anti_alias: true, ..Paint::default() }
}
...
fn rrect_path(x0: f64, y0: f64, x1: f64, y1: f64, r: f64) -> Option<Path> {
	...
	if r <= 0.0 {
		pb.push_rect(tiny_skia::Rect::from_ltrb(x0 as f32, y0 as f32, x1 as f32, y1 as f32)?);
		return pb.finish();
	}
	let k = r * KAPPA;
	...
	if r.stroke_sides != 15 {
		...
		for &(bit, (a, b, c, d)) in &segs {
			...
			if r.has_dash {
				let mut pb = PathBuilder::new();
				if bit == 1 || bit == 4 {
					pb.move_to(a as f32, f64::midpoint(b, d) as f32);
					pb.line_to(c as f32, f64::midpoint(b, d) as f32);
				} else {
					pb.move_to(f64::midpoint(a, c) as f32, b as f32);
					pb.line_to(f64::midpoint(a, c) as f32, d as f32);
				}
				if let Some(path) = pb.finish() {
					let stroke = Stroke { width: w_px as f32, dash: dash.clone(), ..Stroke::default() };
		...
		pb.close();
		if let Some(path) = pb.finish() {
			let stroke = Stroke { width: (r.stroke_w * s) as f32, dash, ..Stroke::default() };
			pix.stroke_path(&path, paint, &stroke, Transform::identity(), clip);
		}
	} else {
		let r_adj = r.radius.mul_add(s, -off).max(0.0);
		if let Some(path) = shape_path(sx0, sy0, sx1, sy1, r_adj, r.smooth) {
			let stroke = Stroke { width: (r.stroke_w * s) as f32, ..Stroke::default() };
			pix.stroke_path(&path, paint, &stroke, Transform::identity(), clip);
		}
	}
}
...
const fn pcg2d(ix: u32, iy: u32) -> u32 {
...
fn square_to_quad(q: [(f64, f64); 4]) -> [f64; 9] {
	...
	let den = dx2.mul_add(-dy1, dx1 * dy2);
	if den == 0.0 {
		return [f64::NAN; 9];
	}
	let g = py.mul_add(-dx2, px * dy2) / den;
	let h = dy1.mul_add(-px, dx1 * py) / den;
	[
		g.mul_add(q[1].0, q[1].0 - q[0].0),
		h.mul_add(q[3].0, q[3].0 - q[0].0),
		q[0].0,
		g.mul_add(q[1].1, q[1].1 - q[0].1),
		h.mul_add(q[3].1, q[3].1 - q[0].1),
		q[0].1,
		g,
		h,
		1.0,
...
fn adjugate(m: [f64; 9]) -> [f64; 9] {
	[
		m[5].mul_add(-m[7], m[4] * m[8]),
		m[1].mul_add(-m[8], m[2] * m[7]),
		m[2].mul_add(-m[4], m[1] * m[5]),
		m[3].mul_add(-m[8], m[5] * m[6]),
		m[2].mul_add(-m[6], m[0] * m[8]),
		m[0].mul_add(-m[5], m[2] * m[3]),
		m[4].mul_add(-m[6], m[3] * m[7]),
		m[0].mul_add(-m[7], m[1] * m[6]),
		m[1].mul_add(-m[3], m[0] * m[4]),
	]
}
...
fn quad_to_quad(from: [(f64, f64); 4], to: [(f64, f64); 4]) -> Option<[f64; 9]> {
	...
	let adj = adjugate(a);
	let det = a[2].mul_add(adj[6], a[1].mul_add(adj[3], a[0] * adj[0]));
	if det == 0.0 {
		return None;
	}
	Some(mat3_mul(b, adj))
...
enum LayerKind {
	Base,
	Group { opacity: f64, blur: f64, mask: Option<GroupMask> },
	Rotate { deg: f64, cx: f64, cy: f64 },
	Scale { cx: f64, cy: f64, sx: f64, sy: f64 },
	Tilt { cx: f64, cy: f64, rx: f64, ry: f64, depth: f64 },
}
...
fn tilt_composite(
	...
	let project = |x: f64, y: f64| -> (f64, f64) {
		...
		let x1 = px * cry;
		let z1 = -px * sry;
		let y2 = z1.mul_add(-srx, py * crx);
		let z2 = z1.mul_add(crx, py * srx);
		let zc = z2.min(0.95 * depth);
		let k = depth / (depth - zc);
		(x1.mul_add(k, cx), y2.mul_add(k, cy))
	};
	let src_q = [(0.0, 0.0), (w, 0.0), (w, h), (0.0, h)];
	let dst_q = [project(0.0, 0.0), project(w, 0.0), project(w, h), project(0.0, h)];
	...
	let wc = hm[7].mul_add(ccy, hm[6] * ccx) + hm[8];
	let sdata = src.data();
	...
	for oy in iy0..iy1 {
		for ox in ix0..ix1 {
			...
			let ww = hm[7].mul_add(fy, hm[6] * fx) + hm[8];
			if ww == 0.0 || ww * wc < 0.0 {
				continue;
			}
			let su = (hm[1].mul_add(fy, hm[0] * fx) + hm[2]) / ww;
			let sv = (hm[4].mul_add(fy, hm[3] * fx) + hm[5]) / ww;
			...
			for (k, chan) in px.iter().enumerate() {
				let v = f64::from(pdata[di + k]).mul_add(inv, chan * cov);
...
impl<'a> Raster<'a> {
			...
			2 => {
				...
				let shader = match gr.kind {
					0 => {
						...
						LinearGradient::new(
							Point::from_xy((cx - dx * ln / 2.0) as f32, (cy - dy * ln / 2.0) as f32),
							Point::from_xy((cx + dx * ln / 2.0) as f32, (cy + dy * ln / 2.0) as f32),
							stops,
							SpreadMode::Pad,
							Transform::identity(),
	...
	fn box_shadow(
		...
		let mw = 2.0f64.mul_add(pad, w).ceil() as u32;
		let mh = 2.0f64.mul_add(pad, h).ceil() as u32;
		let Some(mut tmp) = Pixmap::new(mw, mh) else {
			return;
		};
		let Some(path) = shape_path(pad, pad, pad + w, pad + h, (r + spread).max(0.0), smooth) else {
			return;
		};
		let mut paint = base_paint();
		...
		if let Some(inner) = shape_path(
			pad + spread - sdx,
			pad + spread - sdy,
			pad + spread - sdx + 2.0f64.mul_add(-spread, w),
			pad + spread - sdy + 2.0f64.mul_add(-spread, h),
			r,
			smooth,
		) {
			...
			tmp.fill_path(&inner, &clear, FillRule::Winding, Transform::identity(), None);
		}
	...
	fn inset_shadow(
		...
		let mw = 2.0f64.mul_add(band, w).ceil() as u32;
		let mh = 2.0f64.mul_add(band, h).ceil() as u32;
		let Some(mut tmp) = Pixmap::new(mw, mh) else {
			return;
		};
		...
		if let Some(path) =
			shape_path(band + sdx, band + sdy, band + sdx + w, band + sdy + h, r, smooth)
		{
			let mut clear = base_paint();
			clear.set_color(Color::TRANSPARENT);
			clear.blend_mode = tiny_skia::BlendMode::Source;
			tmp.fill_path(&path, &clear, FillRule::Winding, Transform::identity(), None);
		}
		blur_rgba(tmp.data_mut(), mw as usize, mh as usize, rad);
	...
	fn backdrop(
		...
		for band in 0..passes {
			let (blur_i, sat_i, bright_i) = if band_ix.is_some() {
				...
				(blur * a, (sat - 1.0).mul_add(a, 1.0), (bright - 1.0).mul_add(a, 1.0))
			} else {
				(blur, sat, bright)
			};
			...
			if sat_i != 1.0 || bright_i != 1.0 {
				for px in buf.as_chunks_mut::<4>().0 {
					...
					let (br, bg, bb) =
						(px[0] as f64 * 255.0 / a, px[1] as f64 * 255.0 / a, px[2] as f64 * 255.0 / a);
					let luma = 0.0722f64.mul_add(bb, 0.7152f64.mul_add(bg, 0.2126 * br));
					// saturate about luma, then brightness — CSS filter-list
					// order blur() saturate() brightness()
					let adj = |v: f64| {
						((v - luma).mul_add(sat_i, luma) * bright_i)
							.round()
							.clamp(0.0, 255.0)
					};
			...
			if let Some(path) = shape_path(x0, y0, x1, y1, r.radius * s, r.smooth) {
				self.conic_through(
					...
					&|pix, clip| {
						...
						pix.fill_path(&path, &white, FillRule::Winding, Transform::identity(), clip);
					},
				);
			}
		...
		if self.is_conic(r.stroke_kind, r.stroke) {
			self.conic_through(
				...
				&|pix, clip| {
					let mut white = base_paint();
					white.set_color(Color::WHITE);
					rect_stroke_geo(pix, clip, &white, r, s, sx0, sy0, sx1, sy1, hw, off, dash.clone());
				},
			);
		} else if let Some(paint) =
			self.paint(r.stroke_kind, r.stroke, gx0, gy0, gx1 - gx0, gy1 - gy0, r.opacity)
		{
			rect_stroke_geo(
				&mut surf.pix,
				surf.clips.last(),
	...
	fn paint_at(
		...
		let t = match gr.kind {
			0 => {
				...
				(py - (cy - dy * ln / 2.0)).mul_add(dy, (px - (cx - dx * ln / 2.0)) * dx) / ln
			},
			1 => {
	...
	fn conic_through(
		...
		for iy in iy0..iy1 {
			for ix in ix0..ix1 {
				...
				let rgba = self.paint_at(2, handle, ix as f64 + 0.5, iy as f64 + 0.5, bx, by, bw, bh);
				...
				let inv = 1.0 - alpha;
				let src =
					[f64::from(rr) * alpha, f64::from(gg) * alpha, f64::from(bb) * alpha, 255.0 * alpha];
	...
	fn grain(&self, surf: &mut Layer, r: &OpRect, x0: f64, y0: f64, x1: f64, y1: f64) {
		...
		for iy in iy0..iy1 {
			for ix in ix0..ix1 {
				...
				for k in 0..3 {
					let v = f64::from(data[di + k]).mul_add(inv, ink);
					data[di + k] = v.round().min(255.0) as u8;
				}
				let va = f64::from(data[di + 3]).mul_add(inv, 255.0 * alpha);
		...
		if t.color_kind == 2 {
			...
			if self.is_conic(2, t.color) {
				...
				self.conic_through(
					...
					(gx, gy, gw, gh),
					t.opacity,
					(b.x() as f64, b.y() as f64, b.right() as f64, b.bottom() as f64),
					&|pix, clip| {
						let mut white = base_paint();
						white.set_color(Color::WHITE);
						pix.fill_path(&path, &white, FillRule::Winding, Transform::identity(), clip);
	...
	fn center_label(surf: &mut Layer, text: &str, cx: f64, cy: f64, size_px: f64, color: Color) {
		...
		let ybase = size_px.mul_add(0.35, cy);
		let mut paint = base_paint();
	...
	fn draw_image(&self, surf: &mut Layer, im: &slab_kernel::flatten::OpImage) {
		...
		let Some(src_pix) = decoded else {
			...
			let stroke = Stroke { width: s as f32, ..Stroke::default() };
			...
			let src = compiled.map_or("", |image| self.s.str_at(image.src));
			let label = src
				.rsplit('/')
				.next()
				.filter(|l| !l.is_empty())
		...
		let tx = x0 + iw.mul_add(-sx, dw) / 2.0;
		let ty = y0 + ih.mul_add(-sy, dh) / 2.0;
	...
	pub fn render(&mut self, frame: &Frame) -> Result<Pixmap, String> {
		...
		let mut stack: Vec<Layer> =
			vec![Layer { pix: base, clips: Vec::new(), kind: LayerKind::Base }];
		...
		for (op_index, op) in frame.ops.iter().enumerate() {
			match op {
				...
				FrameOp::PathDraw(p) => {
					...
					let dev = (
						p.dx.mul_add(s, bounds.x() as f64 * s),
						p.dy.mul_add(s, bounds.y() as f64 * s),
					...
					if p.stroke_kind != 0 {
						...
						if self.is_conic(p.stroke_kind, p.stroke) {
							...
							self.conic_through(
								...
								dev,
								p.opacity,
								(dev.0 - hw, dev.1 - hw, dev.0 + dev.2 + hw, dev.1 + dev.3 + hw),
								&|pix, clip| {
									let mut white = base_paint();
									white.set_color(Color::WHITE);
				...
				FrameOp::ClipPush(c) => {
					...
					let mut mask = match surf.clips.last() {
						Some(m) => m.clone(),
						None => Mask::new(surf.pix.width(), surf.pix.height()).ok_or("mask alloc")?,
					};
				...
				FrameOp::GroupPush(gp) => {
					...
					stack.push(Layer {
						pix,
						clips: Vec::new(),
						kind: LayerKind::Group { opacity: gp.opacity, blur: gp.blur * s, mask },
					});
				},
				FrameOp::RotatePush(rt) => {
					...
					stack.push(Layer {
						pix,
						clips: Vec::new(),
						kind: LayerKind::Rotate { deg: rt.deg, cx: rt.cx * s, cy: rt.cy * s },
					});
				},
				FrameOp::ScalePush(scale) => {
					...
					for enclosed in frame.ops.iter().skip(op_index + 1) {
						match enclosed {
							...
							FrameOp::Rect(rect) => {
								let pad =
									rect.stroke_w.max(0.0) + if rect.shadow_len > 0 { 64.0 } else { 0.0 };
							...
							FrameOp::PathDraw(path_op) => {
								if let Some(path) = self.slir_path(frame, path_op.path) {
									...
									source_w = source_w
										.max(path_op.dx + f64::from(bounds.x() + bounds.width()) + pad);
									source_h = source_h
										.max(path_op.dy + f64::from(bounds.y() + bounds.height()) + pad);
								}
							},
					...
					let pix = Pixmap::new(layer_w.max(1), layer_h.max(1)).ok_or("layer alloc failed")?;
					stack.push(Layer { pix, clips: Vec::new(), kind: scale_layer_kind(scale, s) });
				},
				FrameOp::TiltPush(tl) => {
					let (layer_w, layer_h) = {
						let parent = stack.last().expect("layer stack has a root");
				...
				FrameOp::GroupPop | FrameOp::RotatePop | FrameOp::ScalePop | FrameOp::TiltPop => {
					...
					match layer.kind {
						LayerKind::Group { opacity, blur, mask } => {
							if blur > 0.0 {
								let (lw, lh) = (layer.pix.width() as usize, layer.pix.height() as usize);
								blur_rgba(layer.pix.data_mut(), lw, lh, blur_rad(blur));
							}
							if let Some(m) = &mask {
								self.apply_group_mask(&mut layer.pix, m);
							}
							let paint = PixmapPaint { opacity: opacity as f32, ..PixmapPaint::default() };
							parent.pix.draw_pixmap(
								0,
								0,
								layer.pix.as_ref(),
						...
						LayerKind::Tilt { cx, cy, rx, ry, depth } => {
...
mod tests {
	...
	fn scale_layer_center_uses_device_pixels() {
		let scale = slab_kernel::flatten::OpScale { cx: 12.0, cy: 7.5, sx: 0.5, sy: 1.25 };
