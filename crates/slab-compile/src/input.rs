...
use slab_kernel::frame::{self as kframe, Instance, ParamValue};
use slab_kernel::slir::{self as kslir, Doc};
...
fn empty_value(kind: u32) -> ParamValue {
    ParamValue {
        kind,
        num: 0.0,
        s: String::new(),
...
fn prepare_list(doc: &Doc, param: usize, raw: &str) -> Result<Prepared, String> {
    ...
    for (index, item) in array.iter().enumerate() {
        ...
        let key = match object.get("key") {
            ...
            Some(_) => {
                return Err(format!(
                    "item {index} field 'key' must be a string or finite number"
                ));
            }
            ...
            Prepared::List { param, len, items } => {
                ...
                for (index, item) in items.iter().enumerate() {
                    ...
                    for (field, field_value) in &item.fields {
                        ...
                        {
                            return Err(
                                "validated list field was rejected by the kernel".to_string()
                            );
                        }
                    }
                }
    ...
    match value {
        ...
        Value::Array(entries) => entries
            ...
            .map(|(index, entry)| {
                ...
                if pair.len() != 2 {
                    return Err(format!(
                        "sets entry {index} must contain exactly two values"
                    ));
                }
                let name = pair[0]
                    .as_str()
