...
fn set_overrides_change_the_grid() {
	...
	assert!(!dump.contains("Pale Green Things"), "old title still present");
...
fn playing_swaps_glyph_opacity_in_frame_ops() {
	...
	assert_ne!(g_pause, g_play, "plain grid must swap the transport glyph with `playing`");
	assert!(g_pause.contains("|>"), "paused grid missing |> glyph:\n{g_pause}");
	assert!(
...
fn bad_set_exits_two() {
	...
	assert!(err.contains("param 'progress': 'banana' is not a percentage"), "stderr: {err}");
	let (code, err) = run_err(&["--set", "nope=1"]);
	assert_eq!(code, 2);
	assert!(err.contains("param 'nope': no such document param"), "stderr: {err}");
	let (code, err) = run_err(&["--set", "playing=maybe"]);
	assert_eq!(code, 2);
	assert!(err.contains("param 'playing': 'maybe' is not a bool"), "stderr: {err}");
}
