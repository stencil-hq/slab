...
fn enter_toggles_play_and_tick_advances_time() {
    ...
    assert!(
        ...
        "title row must stay on track 1:\n{dump}"
    );
    assert!(
...
fn mouse_hover_changes_shuf_cell_bg() {
    ...
    assert_ne!(
        hb, pb,
        "hover must recolor the SHUF cell bg at ({col},{row})"
    );
    assert_eq!(
        hb, 0x1B2E22,
        "settled hover bg must be moss #1B2E22, got #{hb:06X}"
    );
}
...
/// at alpha 0 (`slab_kernel::motion` CSS-transparent semantics), so the SHUF rect
/// bg paint (0xAABBGGRR) runs #00222E1B → #BF222E1B → #FF222E1B: three
...
fn hover_ease_interpolates_shuf_bg_paint() {
    ...
    assert!(
        v0 != v1 && v1 != v2 && v0 != v2,
        "bg paint must ease through three distinct values: \
         t0=#{v0:08X} t0+70=#{v1:08X} t0+500=#{v2:08X}"
    );
    ...
    assert!(
        a0 < a1 && a1 < a2,
        "midpoint alpha {a1:#04X} must sit strictly between {a0:#04X} \
         and {a2:#04X} (t0+70 of a 140ms ease-out)"
    );
}
...
fn click_play_circle_emits_toggle() {
    ...
    assert!(
        dump.contains("2:38"),
        "click-started playback must advance the clock:\n{dump}"
    );
}
...
fn app_flag_is_opt_in() {
    ...
    assert!(
        dump.contains("2:37"),
        "generic mode must keep the declared elapsed param:\n{dump}"
    );
}
