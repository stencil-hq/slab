...
//!
//! Compiles FILE.slab, drives the hand-maintained Rust kernel (`slab-kernel`),
//! and either runs a live crossterm loop or a headless `--script` replay.

use std::{path::PathBuf, process::ExitCode};

use slab_kernel::frame as kframe;

use crate::{app, images, interactive, player, script};
...
fn load(a: &Args, file: &std::path::Path) -> Result<Doc, (u8, String)> {
	...
	Ok(Doc { inst, app, images, warnings })
}

/// `--env` flags as (dark, coarse).
fn env_flags(a: &Args) -> (bool, bool) {
	(a.env.iter().any(|e| e == "dark"), a.env.iter().any(|e| e == "coarse"))
}
...
fn session(
	...
	let term = interactive::Terminal::new().map_err(|e| (1, e))?;
	let mut index = start;
	loop {
		...
		let mut no_host = ();
		let host: &mut dyn app::Host = match doc.app.as_mut() {
			Some(player) => player,
			None => &mut no_host,
		};
		match term
			.run(&mut doc.inst, host, imgpaint, &ui)
			.map_err(|e| (1, e))?
		{
...
fn run(a: &Args) -> Result<(), (u8, String)> {
	...
	if a.script.is_some() || a.dump_after.is_some() {
		...
		let text = script::run(&mut doc.inst, a.script.as_deref(), 16.0, a.ansi, doc.app.as_mut())
			.map_err(|e| (1, e))?;
		match &a.dump_after {
...
/// Runs the `slab-tui` command with caller-supplied arguments.
