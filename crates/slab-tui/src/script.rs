...
pub fn parse(script: &str) -> Result<Vec<Step>, String> {
	...
	for tok in script.split_whitespace() {
		let named = |key: &str, mods: u32| Step::Key { key: key.to_string(), mods };
...
pub fn run(
	...
	for step in &steps {
		match step {
			Step::Key { key, mods } => dispatch_at(inst, t, &app::key_event(key, *mods), &mut signals),
			...
			Step::Click { x, y } => {
				dispatch_at(inst, t, &app::pointer_event(app::E_POINTER_DOWN, *x, *y), &mut signals);
				t += t_step;
				dispatch_at(inst, t, &app::pointer_event(app::E_POINTER_UP, *x, *y), &mut signals);
			},
			Step::Wheel { x, y, dy } => {
				dispatch_at(inst, t, &app::wheel_event(*x, *y, *dy), &mut signals);
			},
			Step::Move { x, y } => {
				dispatch_at(inst, t, &app::pointer_event(app::E_POINTER_MOVE, *x, *y), &mut signals);
			},
			Step::Tick { ms } => {
