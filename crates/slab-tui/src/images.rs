...
fn terminal_supported() -> bool {
	...
	matches!(std::env::var("TERM_PROGRAM").as_deref(), Ok("ghostty" | "WezTerm"))
}
...
impl Images {
	...
	pub fn new(mode: Mode, doc: &slir::Doc, embedded: &[Vec<u8>], base_dir: &Path) -> Self {
		...
		Self { enabled, data, transmitted, placed: Vec::new() }
	}
...
fn base64(bytes: &[u8]) -> String {
	...
	for chunk in bytes.chunks(3) {
		...
		out.push(char::from(TABLE[usize::try_from(word >> 18).expect("6-bit index")]));
		out.push(char::from(TABLE[usize::try_from((word >> 12) & 63).expect("6-bit index")]));
