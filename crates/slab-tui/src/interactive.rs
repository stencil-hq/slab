...
impl Painter {
	...
	pub const fn with_truecolor(truecolor: bool) -> Self {
		Self {
			truecolor,
			prev: None,
			buf: String::new(),
	...
	pub const fn invalidate_cursor(&mut self) {
		self.cur_pos = None;
	}
}
...
const fn accepts_printable_text(mods: u32) -> bool {
			...
			Event::Mouse(MouseEvent { kind, column, row, modifiers }) => {
				...
				let mut event = match kind {
					...
					MouseEventKind::Up(button) => {
						app::pointer_button_event(app::E_POINTER_UP, x, y, mouse_button_code(button), 0)
					},
					MouseEventKind::Drag(button) => {
						app::pointer_button_event(app::E_POINTER_MOVE, x, y, mouse_button_code(button), 0)
					},
...
impl Gallery<'_> {
	...
	fn footer(&self) -> String {
		let name = self.files[self.index]
			...
			.unwrap_or_default()
			.to_string_lossy();
		format!("[{}/{}] {name}   ^N next   ^P prev   ^C quit", self.index + 1, self.files.len())
	}
}
...
fn handle(
	...
	if let Event::Key(KeyEvent { code: KeyCode::Char(c @ ('n' | 'p')), modifiers, kind, .. }) =
		&event
