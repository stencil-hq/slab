...
fn drive_stdio_smoke() {
    ...
    assert_eq!(
        decode_b64(data["data"].as_str().expect("runtime image bytes")),
        [17, 34, 51, 255]
    );
    assert_eq!(
        result(request("img.unregister", json!({"name": "smoke"}))),
        json!({"ok": true})
    );
    ...
    let scrolled = result(request(
        "scroll.set",
        json!({"key": scroll_key, "axis": 0, "off": 40}),
    ));
