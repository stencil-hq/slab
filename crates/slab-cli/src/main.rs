...
fn cmd_check(args: &[String]) -> ExitCode {
	let p = match parse_args(args, &["width", "height", "state", "env", "client", "renderer"]) {
		Ok(p) => p,
		Err(e) => {
			eprintln!("error: {e}");
			return ExitCode::from(2);
...
/// `slab fmt FILE... [--check]` — canonical formatting via
/// `slab_syntax::format`. `-` reads stdin and writes the result to stdout.
/// `--check` writes nothing and exits 1 when any file would change.
