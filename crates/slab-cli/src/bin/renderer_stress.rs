...
fn populate_graph(instance: &mut Instance, rows: u32) -> Result<(), String> {
    ...
    let lane = value(
        PARAM_TEXT,
        0.0,
        "M 0 13 L 18 13 C 22 13 22 5 28 5 L 94 5".to_owned(),
    );
    ...
    for index in 0..ROW_COUNT {
        ...
        set_field(
            ...
            &value(
                PARAM_TEXT,
                0.0,
                format!("Retained renderer change {index:05}"),
            ),
        )?;
        set_field(
            instance,
            ...
            &value(
                PARAM_TEXT,
                0.0,
                "Reuse layout state and cached paint records".to_owned(),
            ),
        )?;
        set_field(
            instance,
        ...
        set_field(
            ...
            index,
            "sha",
            &value(PARAM_TEXT, 0.0, format!("{index:08x}")),
        )?;
...
fn update_frame(
    ...
    if !inst_set_param(
        instance,
        message_width,
        &value(PARAM_NUM, width, String::new()),
    ) {
    ...
    if !inst_frame_update(
        instance,
        f64::from(u32::try_from(step).expect("step exceeds u32")),
        frame,
    ) {
...
fn trimmed_mean(samples: &[u64]) -> u64 {
    ...
    let sum = middle.iter().fold(0_u128, |total, &sample| {
