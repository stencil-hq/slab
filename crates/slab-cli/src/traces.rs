...
fn drain_frame_signals(
    ...
    lines.push(slab_kernel::dumpjson::dump_effects(
        &inst.doc, &inst.st, &effects,
    ));
}
...
fn run_trace(bytes: &[u8], case: &serde_json::Value) -> TraceResult {
    ...
    for step in &steps {
        ...
        if step["event"].is_object() {
            ...
            lines.push(format!(
                "{{\"set\":\"scroll\",\"changed\":{changed},\"read\":{read}}}"
            ));
            ...
            let ok = match op {
                ...
                "field" => {
                    ...
                    slab_kernel::frame::inst_set_list_field(
                        &mut inst, param, path, index, field, &value,
                    )
                }
                "key" => {
                    let index = json_i32(&list["index"]).unwrap_or(i32::MIN);
            ...
            lines.push(format!(
                "{{\"set\":\"divider\",\"changed\":{changed},\"read\":{read}}}"
            ));
...
fn check_expect(
    ...
    if let Some(edits) = expect["edits"].as_array() {
        for e in edits {
            ...
            if !found {
                return Err(format!(
                    "edit {name:?}: expected {text:?}, summary {}",
                    summary["edits"]
                ));
            }
        }
...
pub fn run_traces(root: &Path, emit_dir: &Path, update: bool) -> (usize, usize) {
    ...
    for name in &names {
        ...
        match std::fs::read_to_string(&golden) {
            ...
            Err(e) => {
                eprintln!(
                    "FAIL trace {name}: {}: {e} (run with --update)",
                    golden.display()
                );
                fail += 1;
            }
