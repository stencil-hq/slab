...
//! | `focus.get` | none | `{"key":str,"visible":bool}`; an empty key means none |
...
use crate::{compile_file, render::load_registered_fonts, traces};
...
fn serve(session: &mut Session, mut input: impl BufRead, mut output: impl Write) -> io::Result<()> {
...
fn focus_get(session: &mut Session) -> ProtocolResult {
    let doc = ensure_frame(session)?;
    Ok(json!({
        "key": scene::key_of(&doc.inst.doc, &doc.inst.st.lists, doc.inst.ds.fs.focus),
...
fn img_register(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
    let image = traces::runtime_image_input(object).map_err(invalid)?;
    let doc = ensure_frame(session)?;
...
fn list_set_field(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
    ...
    let value = traces::typed_value_parts(kind, raw).map_err(invalid)?;
    let doc = ensure_frame(session)?;
    let param = list_param(&doc.inst, &name)?;
...
fn input_event(session: &mut Session, value: &Value) -> ProtocolResult {
    ...
    let event = traces::build_event(value).map_err(invalid)?;
...
fn input_click(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
    ...
    effects_result(doc, &effects, t_ms)
}
