...
//! Slab Drive Protocol (SDP): deterministic NDJSON automation for one live
//! document.
//!
...
//! [`Effects`] through the host's normal signal handler.
//! [`RequestPump::request`] is kernel-only;
//! [`RequestPump::request_with_host_input`] lets host keyboard layers observe
//! and consume SDP key/text input before kernel dispatch. Host-mounted document
//! replacement is denied by default. An opted-in reload
...
//! their expanded definition root. See `spec/SDP.md` for the normative
//! protocol.
//!
...
impl Session {
	const fn new(env: EnvSpec, fonts: Vec<(String, PathBuf)>) -> Self {
		Self {
			fonts,
			env,
			doc: None,
...
impl RequestPump {
	...
	pub const fn with_reload_policy(mut self, policy: ReloadPolicy) -> Self {
	...
	/// Applies one request while letting the host observe and consume key/text
	/// input.
	///
...
fn compile_file(path: &Path, embed_assets: bool) -> (Option<Slir>, Diagnostics) {
	let src = match std::fs::read_to_string(path) {
		...
		Err(error) => {
			...
			diagnostics.error("parse", format!("cannot read {}: {error}", path.display()), 0);
...
fn load_registered_fonts(fonts: &[(String, PathBuf)]) -> Result<Vec<RegisteredFont>, String> {
	fonts
		...
		.map(|(name, path)| {
			...
			Ok(RegisteredFont { name: name.clone(), bytes, metrics })
		})
		.collect()
}
...
pub fn cmd_drive(args: &[String]) -> ExitCode {
	...
	let served = match args.port {
		None => serve_session(&mut session, std::io::stdin().lock(), std::io::stdout().lock()),
...
pub fn reject_busy(stream: TcpStream) {
	let response =
		error_response(None, ERR_DOMAIN, "session busy: another client holds this SDP session");
...
fn doc_info(session: &mut Session) -> ProtocolResult {
	...
	for (index, name_ref) in kernel.parm_name.iter().copied().enumerate() {
		...
		parameter.insert("name".into(), Value::String(slir::str_at(kernel, name_ref).to_owned()));
...
const fn param_type_name(kind: u32) -> &'static str {
	match kind {
		0 => "text",
		1 => "num",
		2 => "pct",
...
const fn client_name(code: u32) -> &'static str {
	match code {
		0 => "web",
		1 => "gpu",
		2 => "tui",
...
fn env_set(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	...
	let environment_changed =
		width.is_some() || height.is_some() || client.is_some() || dark.is_some() || coarse.is_some();
...
fn param_set(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	let sets = match (object.get("sets"), object.get("name"), object.get("value")) {
		...
		_ => {
			return Err(invalid("param.set needs either 'name' and 'value', or 'sets'"));
		},
	};
	let doc = ensure_frame(session)?;
...
fn scroll_set(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	...
	if !frame::inst_set_scroll(&mut doc.inst, &key, axis, off) {
		return Err(domain(format!("key '{key}' is not scrollable on axis {axis}")));
	}
	doc.fr = frame::inst_frame(&mut doc.inst, t_ms);
	Ok(json!({
		 "axis": axis,
...
fn list_set_len(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	...
	if !frame::inst_set_list_len(&mut doc.inst, param, &path, n) {
		return Err(domain(format!("cannot resize parameter '{name}' at path '{path}'")));
	}
	Ok(json!({"ok": true}))
}
...
fn list_set_key(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	...
	if !frame::inst_set_list_key(&mut doc.inst, param, &path, index, &key) {
		return Err(domain(format!("cannot set item key on parameter '{name}' at path '{path}'")));
	}
	Ok(json!({"ok": true}))
}
...
fn list_reveal_item(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	...
	if !frame::inst_reveal_item(&mut doc.inst, &each, index, align) {
		return Err(domain(format!("key '{each}' is not a virtual each containing item {index}")));
	}
	doc.fr = frame::inst_frame(&mut doc.inst, t_ms);
	Ok(json!({"ok": true}))
}
...
fn resolve_node_key(doc: &LoadedDoc, query: &str) -> ProtocolResult<(u32, String)> {
	match scene::resolve_key(&doc.inst.doc, &doc.inst.st.lists, query) {
		...
		scene::KeyResolution::Missing { candidates } => {
			Err(domain(format!("unknown key '{query}'; nearest: {}", quoted_candidates(&candidates))))
		},
	}
}
...
fn scene_node(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	...
	states.insert("hover".into(), Value::Bool(doc.inst.ds.hover.contains(&node)));
...
const fn base_event(etype: u32) -> Event {
	Event {
		etype,
		x: 0.0,
		y: 0.0,
...
fn input_click(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	...
	if key.is_some() && (has_x || has_y) || key.is_none() && !(has_x && has_y) {
		return Err(invalid("input.click needs either 'key' or both 'x' and 'y'"));
	}
	let key = key
		.map(|value| {
			value
	...
	for etype in [dispatch::E_POINTER_MOVE, dispatch::E_POINTER_DOWN, dispatch::E_POINTER_UP] {
...
fn dispatch_input(
	...
	let host_event = match event.etype {
		dispatch::E_KEY_DOWN => Some(PumpHostEvent::Key { key: &event.key, mods: event.mods }),
		dispatch::E_TEXT | dispatch::E_PASTE => {
			Some(PumpHostEvent::Text { text: &event.text, paste: event.etype == dispatch::E_PASTE })
		},
		_ => None,
	};
	...
	frame
		...
		.map(|diagnostic| {
			if diagnostic.line == 0 {
				...
				format!("note {} line {}: {}", diagnostic.code, diagnostic.line, diagnostic.msg)
			}
		})
		.collect()
}
...
fn render_apng(session: &mut Session, object: &Map<String, Value>) -> ProtocolResult {
	...
	{
		let LoadedDoc { slir, inst, images, fonts, .. } = doc;
...
"#,
	...
	fn pumps_field_param_and_focus_methods_on_caller_instance() {
		...
		let field =
			pump.request(&mut instance, r#"{"id":3,"method":"field.get","params":{"key":"field"}}"#);
		assert_eq!(result(&field), &json!({"text": "edited"}));

		let _ =
			pump.request(&mut instance, r#"{"id":4,"method":"focus.set","params":{"key":"field"}}"#);
	...
	fn returns_dispatch_effects_for_host_handling() {
		...
		let mut pump = RequestPump::new("test.slab", slir, images);
		let _ =
			pump.request(&mut instance, r#"{"id":1,"method":"focus.set","params":{"key":"field"}}"#);
		let response =
...
"#,
		...
		let mut pump = RequestPump::new("test.slab", slir, images);
		let response =
			pump.request(&mut instance, r##"{"method":"input.click","params":{"key":"#theme"}}"##);
		let signals = result(&response)["effects"]["signals"]
			.as_array()
			.expect("input result signals");
		assert!(signals.iter().any(|signal| signal["name"] == "theme"), "{:?}", response.response);
	}
...
"#,
		...
		let wrong_kind =
			pump.request(&mut instance, r##"{"method":"list.window","params":{"each":"#viewport"}}"##);
		assert!(
...
"#,
		...
		let mut pump = RequestPump::new("test.slab", slir, images);
		let ambiguous =
			pump.request(&mut instance, r##"{"method":"input.click","params":{"key":"#go"}}"##);
		...
		let missing =
			pump.request(&mut instance, r##"{"method":"input.click","params":{"key":"#goo"}}"##);
	...
	fn render_notes_preserve_ordered_frame_diagnostics() {
		...
		assert_eq!(frame_diagnostic_notes(&frame), [
...
",
		...
		let read_back =
			pump.request(&mut instance, r#"{"method":"param.get","params":{"name":"progress"}}"#);
...
"#,
		...
		assert_eq!(result(&revealed), &json!({"ok": true}), "{:?}", revealed.response);
...
"#,
		...
		let mut pump = RequestPump::new("test.slab", slir, images);
		let response =
			pump.request(&mut instance, r##"{"method":"input.click","params":{"key":"#fal"}}"##);
		let message = response.response["error"]["message"]
			.as_str()
			.expect("missing key error");
		let nearest = message
