//! `slab-macro` — compile `.slab` documents into typed Rust modules at build
//! time.
//!
//! The [`include_doc!`] macro runs the slab compiler at macro expansion time
//! and splices the generated typed module (a `Doc` wrapper over
//! `slab_kernel::frame::Instance`, `PARAM_*` consts, and a `Signal` enum) into
//! the invoking crate. Compile failures in the document surface as real slab
...
/// Compile `path` (relative to `CARGO_MANIFEST_DIR`) with `fonts` overriding
/// the bundled faces, and assemble the wrapping `pub mod` source, or a
...
fn expand(path: &str, name: Option<&str>, fonts: &[(String, String)]) -> Result<String, String> {
	...
	let name = match name {
		...
		None => module_name(
			abs.file_stem()
				.and_then(|s| s.to_str())
				.ok_or_else(|| format!("include_doc!: `{}` has no usable file stem", abs.display()))?,
		),
	};
...
mod tests {
	...
	fn custom_font_compiles_and_is_tracked() {
		let fonts = [("Inter".to_owned(), "../../assets/fonts/Inter-Regular.ttf".to_owned())];
