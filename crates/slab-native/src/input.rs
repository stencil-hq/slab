...
use slab_kernel::dispatch::{self as kdispatch, Effects};
use slab_kernel::edit;
...
mod input_tests {
    ...
    fn preedit_then_commit_brackets_a_composition() {
        ...
        assert_eq!(
            events,
            vec![(kdispatch::E_COMPOSITION_UPDATE, "にほ".to_string())]
        );

        let events = ime.on_ime(Ime::Commit("日本".into()));
        assert_eq!(
            events,
            vec![(kdispatch::E_COMPOSITION_END, "日本".to_string())]
        );
        assert!(!ime.composing());
    }
