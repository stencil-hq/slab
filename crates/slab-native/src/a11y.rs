...
use slab_kernel::{
	dispatch::{self as kdispatch, Event as KernelEvent},
...
enum RoutedActionKind {
	...
	Reveal,
	ScrollBy { axis: u32, delta: f64 },
	SetScroll { main: Option<f64>, cross: Option<f64> },
}
...
impl Default for Snapshot {
	fn default() -> Self {
		Self { valid: false, nodes: BTreeMap::new(), routes: HashMap::new(), focus: ROOT_ID }
	}
}

/// Pure scene-to-AccessKit state. It is tested without constructing an OS
/// adapter.
pub struct Bridge {
...
impl Bridge {
	...
	pub fn refresh(
		...
		for layer in layers {
			...
			for source in &layer.frame.scene {
				...
				let identity = if key.is_empty() {
					...
					Identity::Key { document: layer.document, key: key.clone() }
				};
		...
		root.set_bounds(Rect { x0: 0.0, y0: 0.0, x1: width.max(0.0), y1: height.max(0.0) });
		...
		for (layer_index, layer) in layers.iter().enumerate() {
			for (scene_index, source) in layer.frame.scene.iter().enumerate() {
				...
				let bounds =
					transformed_bounds(&layer.frame.scene, scene_index, layer.offset_x, layer.offset_y);
	...
	pub fn prepare_update(&self, force_full: bool) -> Option<TreeUpdate> {
		...
		Some(TreeUpdate { nodes, tree, tree_id: TreeId::ROOT, focus: source.focus })
	}
		...
		let kind = match request.action {
			...
			Action::SetScrollOffset => {
				...
				let (main, cross) = if target.scroll.is_row {
					(target.scroll.main.then_some(point.x), target.scroll.cross.then_some(point.y))
				} else {
					(target.scroll.main.then_some(point.y), target.scroll.cross.then_some(point.x))
				};
				RoutedActionKind::SetScroll { main, cross }
			},
			_ => return None,
		};
		Some(RoutedAction { document: target.document, key: target.key.clone(), kind })
	}
}
...
fn scroll_action(
	...
	Some(RoutedActionKind::ScrollBy { axis, delta: direction * amount })
}
...
fn apply_scroll_properties(node: &mut Node, source: &SceneNode, enabled: bool) -> ScrollTarget {
	...
	ScrollTarget { is_row: source.is_row, main, cross, main_viewport, cross_viewport }
}
...
mod tests {
	...
	fn role_and_state_mapping_preserves_absent_false_true_and_mixed() {
		...
		assert_eq!(role_from_name("vendor-widget", slir::K_RECT, 0), Role::Unknown);
	...
	fn nested_rotation_and_layer_offset_are_projected_before_root_physical_scale() {
		...
		for (actual, expected) in
			[(bounds.x0, 95.0), (bounds.y0, 60.0), (bounds.x1, 100.0), (bounds.y1, 70.0)]
		{
	...
	fn stale_disabled_inert_or_nonfocusable_kernel_focus_normalizes_to_root() {
		...
		for (flags, disabled) in
			[(slir::F_FOCUSABLE, true), (slir::F_FOCUSABLE | slir::F_INERT, false), (0, false)]
		{
			...
			let mut bridge = Bridge::default();
			bridge.refresh("test", 100.0, 100.0, 1.0, &[SceneLayer::new(3, &instance, &frame)]);
	...
	fn stable_ids_hierarchy_and_incremental_updates_follow_scene_keys() {
		...
		bridge.refresh("test", 200.0, 100.0, 2.0, &[SceneLayer::new(7, &instance, &moved)]);
	...
	fn stable_queue_document_identity_survives_backing_instance_selection() {
		...
		let mut bridge = Bridge::default();
		bridge.refresh("test", 100.0, 100.0, 1.0, &[SceneLayer::new(
			1,
			&first_instance,
			&first_frame,
		)]);
	...
	fn action_resolution_returns_exact_document_and_key_without_os_adapter() {
		...
		let mut bridge = Bridge::default();
		bridge.refresh("test", 200.0, 100.0, 1.0, &[SceneLayer::new(42, &instance, &frame)]);
		let snapshot = bridge.source().unwrap();
		...
		let page_right = bridge
			...
			.unwrap();
		assert_eq!(page_right.kind, RoutedActionKind::ScrollBy { axis: 0, delta: 100.0 });
		...
		let set_scroll = bridge
			...
			.unwrap();
		assert_eq!(set_scroll.kind, RoutedActionKind::SetScroll { main: Some(12.0), cross: None });
