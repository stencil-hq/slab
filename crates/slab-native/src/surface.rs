...
//! - `Occluded` skips quietly; hosts repaint from `WindowEvent::Occluded(false)`
//!   rather than spinning while hidden.
//! - At most one synchronous draw per `Resized` event: transactional drawables
//!   return to the pool only when the resize transaction commits, so queueing
//!   a second render per tick starves `nextDrawable` and stalls the drag.
