...
use slab_kernel::squircle::squircle_path;
use slab_kernel::{flatten::OpRect, slir::Doc};
...
fn lyon_path_data(verbs: impl IntoIterator<Item = u32>, coords: &[f64]) -> Option<Path> {
    ...
    let point = |index: usize| {
        Some(Point::new(
            *coords.get(index)? as f32,
            *coords.get(index.checked_add(1)?)? as f32,
        ))
    };
    for verb in verbs {
        match verb {
...
pub fn squircle_fill_mesh(
    ...
    fill_path(
        device,
        &squircle_lyon_path(w, h, r, smooth, 0.0, 0.0)?,
        "squircle-fill",
    )
}
