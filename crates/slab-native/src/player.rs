...
use slab_kernel::scene as kscene;
    ...
    fn dispatch_queue(&mut self, ev: &Event) -> CoreOut {
        ...
        let (repaint, cursor, picked_node, signals) = {
            ...
            (
                effects.repaint,
                effects.cursor,
                picked.then_some(queue.inst.ds.fs.focus),
                signals,
            )
        };
        out.repaint = repaint;
...
impl App {
    ...
    fn refresh_accessibility(
        ...
        layers.push(a11y::SceneLayer::new(
            A11Y_MAIN_DOCUMENT,
            &self.core.doc.inst,
            frame,
        ));
        if let Some(rect) = self.core.hole_rects.first() {
            let mut layer = a11y::SceneLayer::new(
                A11Y_QUEUE_DOCUMENT,
                &self.core.queues[idx].inst,
                queue_frame,
            )
    ...
    fn accessibility_action(&mut self, request: &accesskit::ActionRequest) {
        ...
        let (route, result) = match routed.document {
            ...
            A11Y_QUEUE_DOCUMENT if idx < self.core.queues.len() => (
                Route::Hole(0),
                routed.apply(&mut self.core.queues[idx].inst),
            ),
            _ => return,
        };
