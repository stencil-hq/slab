...
use slab_kernel::scene;
use slab_native::gen_player::{Doc, Signal};
use slab_native::player::PlayerCore;
...
fn playing_param_swaps_transport_glyph() {
    ...
    let fr = doc.frame(0.0);
    assert!(
        glyph_opacity(&fr, "|>") < 0.01,
        "|> should be hidden while playing"
    );
    assert!(
        glyph_opacity(&fr, "||") > 0.99,
        "|| should be visible while playing"
    );
    ...
    assert!(
        glyph_opacity(&fr, "|>") > 0.99,
        "|> should be visible while paused"
    );
    assert!(
        glyph_opacity(&fr, "||") < 0.01,
        "|| should be hidden while paused"
    );
}
...
fn progress_param_moves_playhead_knob() {
    ...
    assert!(
        (got - want).abs() <= 2.0,
        "knob moved {got}u for a 60% progress step, want ~{want}u"
    );
}
...
fn queue_click_selects_track() {
    ...
    assert!(
        text_present(&fr, "This Year"),
        "main frame should title the picked track"
    );
...
fn queue_selection_preserves_keyed_focus_and_keyboard_owner() {
    ...
    assert_eq!(
        activated.picked,
        Some(1),
        "Enter must remain routed to the focused queue row"
    );
}
...
fn queue_row_hover_and_press_ease_bg() {
    ...
    assert_eq!(
        out.cursor,
        kdispatch::CUR_POINTER,
        "queue rows are focusable -> pointer cursor"
    );
