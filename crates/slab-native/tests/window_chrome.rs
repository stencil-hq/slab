...
fn undecorated_state_gates_chrome_and_signals_map() {
    ...
    assert_ne!(
        bar,
        slab_kernel::slir::NONE,
        "titlebar missing from doc tree"
    );
    assert!(
    ...
    assert!(
        scene::index_of(&inst.sc, close) >= 0,
        "no close control in scene"
    );

    // reserved-name mapping straight off the act= bindings
    assert_eq!(
        act_signal(&inst.doc, bar).and_then(WindowCmd::from_signal),
        Some(WindowCmd::Drag)
    );
    assert_eq!(
