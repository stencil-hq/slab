...
fn probes_gpu_dashes_and_inset_shadow() {
    ...
    check(
        ...
        &[
            ...
            Probe {
                ...
                y: 109,
                want: [70, 75, 82],
                what: "inset shadow rim",
            },
            Probe {
                x: 20,
                y: 120,
                want: [0x20, 0x26, 0x2E],
                what: "inset shadow center",
            },
        ],
    ...
    let src = "stack w=100 h=30 bg=#101418 {\n\
                 path \"M10 15 L90 15\" stroke=#ffffff stroke-w=2 stroke-dash=6,4\n\
               }\n";
...
fn probes_text_coverage() {
    ...
    println!(
        "l1-text: {lit} lit pixels in [{x0},{x1})x[{y0},{y1}) (box {}x{})",
        x1 - x0,
        y1 - y0
    );
    assert!(
        lit > 200,
        "expected >200 glyph pixels inside the first Text op box, got {lit}"
    );
}
