...
pub fn test_boundary_navigation() {
    ...
    assert_eq!(
        graphemes::prev_boundary(&boundaries, 3),
        1,
        "prev skips mark"
    );
    assert_eq!(graphemes::prev_boundary(&boundaries, 1), 0, "prev to start");
    assert_eq!(
        graphemes::next_boundary(&boundaries, 1, 4),
        3,
        "next skips mark"
    );
    assert_eq!(
        graphemes::next_boundary(&boundaries, 3, 4),
        4,
        "next to end"
    );
    ...
    assert!(
        graphemes::is_ri(0x1F1E6) && !graphemes::is_ri(0x1F1E5),
        "RI range edges"
    );
}
