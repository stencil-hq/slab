...
pub fn test_math_intrinsics() {
    ...
    assert!(approx(9.0_f64.powf(0.5), 3.0, 1.0e-12), "pow(9, 0.5) == 3");
}
...
pub fn test_lerp() {
    assert_eq!(
        color::lerp_oklab(0xFF0000FF, 0x0000FFFF, 0.0),
        0xFF0000FF,
        "t=0 endpoint"
    );
    assert_eq!(
        color::lerp_oklab(0xFF0000FF, 0x0000FFFF, 1.0),
        0x0000FFFF,
        "t=1 endpoint"
    );
    assert_eq!(
        color::lerp_oklab(0x000000FF, 0xFFFFFFFF, 0.5),
        0x636363FF,
        "black/white midpoint"
    );
    assert_eq!(
        color::lerp_oklab(0xFF0000FF, 0x0000FFFF, 0.5),
        0x8C53A2FF,
        "red/blue midpoint"
    );
    assert_eq!(
        color::lerp_oklab(0x336699FF, 0xFF8000FF, 0.25),
        0x6E7387FF,
        "steel/orange quarter"
    );
    assert_eq!(
        color::lerp_oklab(0x11223300, 0x112233FF, 0.5),
        0x11223380,
        "alpha midpoint"
    );
}
