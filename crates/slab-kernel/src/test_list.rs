...
pub fn list_doc() -> slir::Doc {
	...
	// `when shown` makes item 0 inert; `when selected` applies scrolling per
	// synthetic id.
	d.cond_kind
...
pub fn test_list_prop_patch_state_isolation_focus_and_content() {
	...
	assert_eq!(style::eff_flags(&d, &st, first) & slir::F_SCROLL, 0, "state absent on sibling item");
	assert_ne!(style::eff_flags(&d, &st, second) & slir::F_SCROLL, 0, "state follows synthetic id");
	style::set_patch_flags(&d, &mut st, first, 100.0, 100.0);
	style::set_patch_flags(&d, &mut st, second, 20.0, 100.0);
	assert_ne!(style::eff_flags(&d, &st, first) & slir::F_CLIP, 0, "width patch on first item");
	assert_eq!(
...
pub fn test_list_keyed_reorder_identity_prune_and_key_addressing() {
	...
	assert_eq!(list::set_len(&d, &mut st.lists, 0, 1), 1, "truncate keyed list");
	style::begin_solve(&d, &mut st);
	assert!(!style::node_state_on(&d, &st, a, "selected"), "truncate drops node state");
	assert_eq!(style::scroll_get(&st, a), 0.0, "truncate drops scroll state");
	assert_eq!(style::field_scroll_x(&st, a), 0.0, "truncate drops field scroll state");
	assert_eq!(list::base(&st.lists, &d, a), slir::NONE, "truncated synthetic id pruned");
	assert_eq!(list::base(&st.lists, &d, b), 1, "remaining keyed id retained");
	assert_eq!(
...
pub fn recursive_list_doc() -> slir::Doc {
	...
	d.node_flags
		.extend([0, slir::F_DETACHED | slir::F_FOCUSABLE, 0, slir::F_FOCUSABLE]);
...
pub fn test_recursive_list_paths_materialization_and_pruning() {
	...
	assert_eq!(
		list::length(&d, &st.lists, list::resolve_path(&d, &st.lists, 0, "0.children.0.children")),
		1,
	...
	assert_eq!(scene::key_of(&d, &st.lists, inner[0]), "tree~root/row/children~child/text");
	let removed = inner[0];
	...
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "", 0, "a"), 1);
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "", 1, "b"), 1);
	assert_eq!(list::set_len_path(&d, &mut reordered.lists, 0, "0.children", 2), 1);
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "0.children", 0, "a0"), 1);
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "0.children", 1, "a1"), 1);
	assert_eq!(list::set_len_path(&d, &mut reordered.lists, 0, "1.children", 1), 1);
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "1.children", 0, "b0"), 1);
	...
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "", 0, "b"), 1);
	assert_eq!(
	...
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "0.children", 0, "b0"), 1);
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "", 1, "a"), 1);
	assert_eq!(list::set_len_path(&d, &mut reordered.lists, 0, "1.children", 2), 1);
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "1.children", 0, "a0"), 1);
	assert_eq!(list::set_key_path(&d, &mut reordered.lists, 0, "1.children", 1, "a1"), 1);
	style::begin_solve(&d, &mut reordered);
	let mut after = Vec::new();
	style::children(&d, &mut reordered, 0, &mut after);
	assert_eq!(after[1], a_root, "parent identity follows its reordered key");
	let mut a_row_after = Vec::new();
	style::children(&d, &mut reordered, after[1], &mut a_row_after);
	assert_eq!(a_row_after[0], a_each, "nested each identity survives reorder");
...
pub fn test_virtual_list_window_extent_identity_and_focus() {
	...
	assert!(list::set_virtual_viewport(&d, &mut st.lists, 1, 100.0, 1000.0, 0.0,));
...
fn virtual_lookup_work_is_independent_of_logical_length() {
	fn solve_work(len: i32) -> usize {
		...
		assert!(list::set_virtual_viewport(&d, &mut st.lists, 1, 100.0, 1_000.0, 0.0,));
...
pub fn test_recursive_list_defaults_and_reextension_are_clean() {
	...
	assert_eq!(list::resolve_path(&d, &st.lists, 0, "0.children"), slir::NONE);
...
pub fn test_virtual_list_frame_settle_reveal_and_op_bound() {
	...
	assert_eq!(
		...
		"virtual~20/row",
		"focus_item materializes and focuses the first interactive descendant: note={} window={:?} \
		 scroll={}",
...
fn virtual_motion_work_tracks_current_window_and_retains_clock_state() {
	...
	assert!(style::set_node_state(&inst.doc, &mut inst.st, retained, "selected", true));
	inst.dirty = true;
	...
	assert!(style::node_state_on(&inst.doc, &inst.st, retained, "selected"));
	let dewindowed_clock =
		usize::try_from(motion::sp_find(&inst.ms, retained, 0)).expect("de-windowed synthetic clock");
