...
/// Builds a single-hole document with optional row orientation and size
/// constraints.
...
pub fn solve(instance: &mut frame::Instance) -> i32 {
	...
	layout::solve(&instance.doc, &mut instance.st, &mut instance.lay, 500.0, 500.0, true)
}
...
pub fn test_hug_hole_reported_dimensions_in_both_orientations() {
	...
	assert_eq!((col.lay.p_w[second], col.lay.p_h[second]), (37.0, 19.0), "reported size persists");
...
pub fn test_hug_hole_report_is_clamped_by_min_and_max() {
	...
	assert_eq!(instance.lay.p_h[root], 11.0, "max-h ceilings reported height");
}
...
pub fn test_hole_report_does_not_override_non_hug_or_non_hole() {
	...
	assert_eq!(fixed.lay.p_h[root], 19.0, "hug axis still uses reported height");
	...
	assert_eq!((rect.lay.p_w[root], rect.lay.p_h[root]), (0.0, 0.0), "non-hole ignores reports");
}
...
mod wave0_api {
	...
	fn runtime_image_registry_keeps_indices_and_generations_mean_changes() {
		...
		instance.dirty = false;
		assert_eq!(frame::inst_img_register(&mut instance, "avatar", 2, 1, 1, &rgba), image);
		...
		assert_eq!(frame::inst_img_register(&mut instance, "avatar", 2, 1, 1, &changed), image);
	...
	fn runtime_image_registry_rejects_bad_rgba_atomically() {
		...
		assert_eq!(frame::inst_img_register(&mut instance, "bad", 2, 2, 1, &short), -1);
		...
		instance.dirty = false;
		assert_eq!(frame::inst_img_register(&mut instance, "good", 2, 2, 1, &short), -1);
		assert_eq!(frame::inst_img_info(&instance, image), before);
		assert_eq!(frame::inst_img_bytes(&instance, image), before_bytes.as_slice());
	...
	fn runtime_image_registry_rejects_zero_corrupt_and_mismatched_png() {
		...
		assert_eq!(frame::inst_img_register(&mut instance, "zero-png", 0, 1, 0, &png_bytes), -1);
		assert_eq!(frame::inst_img_register(&mut instance, "zero-rgba", 1, 0, 1, &[]), -1);
		assert_eq!(frame::inst_img_register(&mut instance, "corrupt", 1, 1, 0, b"not a png"), -1);
		assert_eq!(frame::inst_img_register(&mut instance, "mismatch", 2, 1, 0, &png_bytes), -1);
		assert!(!instance.dirty, "invalid inputs are rejected atomically");

		assert_eq!(frame::inst_img_register(&mut instance, "valid", 1, 1, 0, &png_bytes), 0);
	...
	fn nested_list_paths_are_unknown_without_nested_schemas() {
		...
		assert!(!frame::inst_set_list_key(&mut instance, 0, "0.children", 0, "child"));
		assert!(!instance.dirty, "unknown paths have no partial side effects");
		assert_eq!(frame::inst_list_len(&instance, 0, ""), 1);
	}
	...
	fn scroll_axes_validate_and_cross_offsets_clamp_after_solve() {
		...
		assert!(instance.dirty, "post-solve clamping schedules a settled frame");
	}
	...
	fn reveal_minimally_scrolls_every_main_axis_ancestor() {
		...
		solved
			.scene
			.push(scene_node(0, -1, slir::K_COL, 0.0, 100.0, slir::F_SCROLL, 400.0));
		solved
			.scene
			.push(scene_node(1, 0, slir::K_COL, 150.0, 100.0, slir::F_SCROLL, 300.0));
		solved
			.scene
	...
	fn controls_without_labels_take_scene_names_from_content() {
		...
		assert!(caption.editable, "an active field binder marks the scene entry editable");
	...
	fn virtual_and_divider_unavailable_cases_are_total() {
		let each_doc =
			keyed_doc(&[slir::K_EACH], &[0], &[slir::NONE], &[slir::NONE], &[slir::NONE], &["items"]);
		...
		let divider_doc =
			keyed_doc(&[slir::K_DIVIDER], &[0], &[slir::NONE], &[slir::NONE], &[slir::NONE], &[
				"orphan",
			]);
		...
		assert!(!divider.dirty, "invalid divider writes have no side effects");
	}
...
mod runtime_image_resolution {
	...
	fn runtime_image_names_override_compiled_and_missing_sources_keep_layout() {
		...
		assert_eq!(missing.scene.len(), 3, "unresolved images keep layout and scene");
		...
		assert_eq!(image_indices(&frame::inst_frame(&mut instance, 0.0)), [runtime, runtime],);
		assert!(frame::inst_img_unregister(&mut instance, "avatar"));
