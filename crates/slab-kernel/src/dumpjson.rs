...
pub fn emit_scene(out: &mut Vec<u32>, fr: &flatten::Frame, index: i32) {
	...
	emit(out, ",\"role\":");
	emit_u32(out, node.sem.role);
	emit(out, ",\"label\":");
	emit_u32(out, node.sem.label);
	emit(out, ",\"desc\":");
	emit_u32(out, node.sem.desc);
	emit(out, ",\"checked\":");
	emit_u32(out, node.sem.checked);
	emit(out, ",\"expanded\":");
	emit_u32(out, node.sem.expanded);
	emit(out, ",\"selected\":");
	emit_u32(out, node.sem.selected);
	emit(out, ",\"active_descendant\":");
	emit_u32(out, node.sem.active_descendant);
	emit(out, ",\"controls\":");
	emit_u32(out, node.sem.controls);
	emit(out, ",\"value_now\":");
	emit_optional_num(out, node.sem.value_now);
	emit(out, ",\"value_min\":");
	emit_optional_num(out, node.sem.value_min);
	emit(out, ",\"value_max\":");
	emit_optional_num(out, node.sem.value_max);
	emit(out, ",\"value_text\":");
	emit_u32(out, node.sem.value_text);
	emit(out, ",\"modal\":");
	emit_u32(out, node.sem.modal);
	emit(out, ",\"live\":");
	emit_u32(out, node.sem.live);
	emit(out, ",\"live_atomic\":");
	emit_u32(out, node.sem.live_atomic);
	emit(out, ",\"level\":");
	emit_optional_num(out, node.sem.level);
	emit(out, ",\"pos_in_set\":");
	emit_optional_num(out, node.sem.pos_in_set);
	emit(out, ",\"set_size\":");
	emit_optional_num(out, node.sem.set_size);
	emit(out, ",\"disabled\":");
...
pub fn dump(d: &slir::Doc, _st: &style::St, fr: &flatten::Frame) -> String {
	...
	for (index, _) in fr.ops.iter().enumerate() {
		...
		emit_op(&mut out, d, fr, i32::try_from(index).expect("operation count fits i32"));
	}
	emit(&mut out, "],\"scene\":[");
	for (index, _) in fr.scene.iter().enumerate() {
		...
		emit_scene(&mut out, fr, i32::try_from(index).expect("scene count fits i32"));
	}
