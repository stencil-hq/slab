...
fn divider_doc(row: bool) -> slir::Doc {
	...
	add_attr(&mut doc, if row { slir::A_W } else { slir::A_H }, slir::T_SIZE_FIXED, 300.0);
	add_attr(&mut doc, if row { slir::A_H } else { slir::A_W }, slir::T_SIZE_FIXED, 80.0);
	finish_attrs(&mut doc);

	add_attr(&mut doc, if row { slir::A_W } else { slir::A_H }, slir::T_SIZE_FILL, 1.0);
	add_attr(&mut doc, if row { slir::A_MIN_W } else { slir::A_MIN_H }, slir::T_NUM, 80.0);
	add_attr(&mut doc, if row { slir::A_MAX_W } else { slir::A_MAX_H }, slir::T_NUM, 220.0);
	finish_attrs(&mut doc);

	add_attr(&mut doc, if row { slir::A_W } else { slir::A_H }, slir::T_SIZE_FIXED, 6.0);
	add_attr(&mut doc, if row { slir::A_MIN_W } else { slir::A_MIN_H }, slir::T_NUM, 0.0);
	...
	add_attr(&mut doc, if row { slir::A_W } else { slir::A_H }, slir::T_SIZE_FILL, 1.0);
	add_attr(&mut doc, if row { slir::A_MIN_W } else { slir::A_MIN_H }, slir::T_NUM, 70.0);
	finish_attrs(&mut doc);
...
/// Verifies the standalone clamp used by host, layout, pointer, and keyboard
/// writes.
...
pub fn test_divider_host_restore_and_layout_bounds() {
	...
	assert!(frame::inst_set_divider(&mut instance, "#root/#split", 190.0));
	frame::inst_frame(&mut instance, 1.0);
	assert_eq!(extent(&instance, 1, true), 190.0);
	assert_eq!(extent(&instance, 3, true), 104.0);

	assert!(frame::inst_set_divider(&mut instance, "#root/#split", 999.0));
	...
	assert_eq!(extent(&instance, 3, true), 74.0, "next minimum is preserved");

	assert!(frame::inst_set_divider(&mut instance, "#root/#split", -10.0));
...
pub fn test_divider_pointer_keyboard_cursor_and_reset() {
	...
	let moved =
		frame::inst_dispatch(&mut instance, &pointer(dispatch::E_POINTER_MOVE, x + 20.0, y, 1));
	assert!(moved.repaint, "drag updates the overlay live");
	assert_eq!(moved.sig_name.as_slice(), &[5], "drag delivers the resize signal live");
	assert_eq!(moved.sig_text[0], "167");
	frame::inst_frame(&mut instance, 1.0);
	assert_eq!(extent(&instance, 1, true), 167.0);

	let released =
		frame::inst_dispatch(&mut instance, &pointer(dispatch::E_POINTER_UP, x + 20.0, y, 1));
	...
	let reset =
		frame::inst_dispatch(&mut instance, &pointer(dispatch::E_POINTER_DOWN, reset_x, reset_y, 2));
	assert_eq!(reset.sig_name.as_slice(), &[6]);
	assert_eq!(frame::inst_get_divider(&instance, "#root/#split"), -1.0);
	let reset_release =
		frame::inst_dispatch(&mut instance, &pointer(dispatch::E_POINTER_UP, reset_x, reset_y, 2));
	assert!(
		reset_release.sig_name.is_empty(),
		"double-click reset does not also emit resize or activate"
	);
	frame::inst_frame(&mut instance, 4.0);
	assert_eq!(extent(&instance, 1, true), 147.0, "reset restores authored fill");
}
...
pub fn test_column_divider_axis_and_cursor() {
	...
	assert!(off_axis.sig_name.is_empty(), "off-axis arrow does not resize");
}

/// Verifies structural siblings remain discoverable after sticky paint
/// reordering.
pub fn test_divider_neighbors_ignore_sticky_paint_order() {
	...
	assert_eq!(released.sig_text.len(), 1, "structural neighbors arm resize");
}
...
pub fn test_disabled_nodes_reject_host_focus_and_tab() {
	...
	instance.doc.node_flags[3] |= slir::F_FOCUSABLE;
	assert!(frame::inst_set_node_state(&mut instance, "#root/#before", "disabled", true));
	frame::inst_frame(&mut instance, 0.0);
	...
	assert_eq!(instance.ds.fs.focus, 2, "Tab starts at the next enabled node");
	frame::inst_dispatch(&mut instance, &key("Tab", 0));
	assert_eq!(instance.ds.fs.focus, 3, "Tab advances to the later sibling");
	frame::inst_dispatch(&mut instance, &key("Tab", 0));
	assert_eq!(instance.ds.fs.focus, 2, "Tab wraps without visiting the disabled node");
}

/// Verifies vanished synthetic divider identities release their retained
/// overlays.
pub fn test_divider_overlay_prunes_with_synthetic_identity() {
	...
	assert_eq!(state.divider_footprint_node.len(), state.divider_footprint.len());
}
...
pub fn test_divider_reserves_nonfixed_handle_footprints() {
	let cases = [
		("percent", slir::T_SIZE_PCT, 10.0, 0.0, 0.0, Some((200.0, 30.0))),
	...
	for (name, size_tag, size_value, minimum, pad_side, expected) in cases {
		...
		assert!(!instance.st.divider_footprint_changed, "{name}: no pending footprint settle");
		...
		assert!((measured - handle).abs() < 1e-9, "{name}: measured footprint");
		if let Some((expected_previous, expected_handle)) = expected {
			assert!((previous - expected_previous).abs() < 1e-9, "{name}: previous extent");
			assert!((handle - expected_handle).abs() < 1e-9, "{name}: handle extent");
		}
	}
}
