...
/// Verifies that a registered runtime font supersedes matching compiled metrics.
pub fn test_runtime_font_register_overrides_matching_family() {
    ...
    assert_eq!(
        slir::font_select(&inst.doc, family, 400),
        registered,
        "family match wins"
    );
    ...
    let layout = textm::measure_text(
        &inst.doc, registered, 10.0, 1.4, 0.0, "A", 100.0, WRAP, ELLIPSIS, -1,
    );
