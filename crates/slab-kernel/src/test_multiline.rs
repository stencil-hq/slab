...
/// Creates an empty fixture from fresh document, style, layout, scene, and dispatch state.
pub fn fix_new() -> Fix {
    Fix {
        d: slir::doc_new(),
        st: style::st_new(),
...
/// Verifies the multiline, submit-bound, modified, and single-line Enter matrix.
pub fn test_enter_matrix_and_submit_payload() {
    ...
    assert_eq!(
        edit::text_str(&a.ds.ed[0]),
        "a\n",
        "multiline Enter inserts newline"
    );
    assert!(
        ea.sig_name.len() == 1 && ea.sig_name[0] == 1,
        "newline emits Change"
    );
    ...
    assert_eq!(
        edit::text_str(&b.ds.ed[0]),
        "draft",
        "plain Enter with submit does not insert"
    );
    assert!(
        eb.sig_name.len() == 1 && eb.sig_name[0] == 2,
        "trigger 2 emitted"
    );
    assert_eq!(eb.sig_text[0], "draft", "Submit carries committed text");
    assert!(
        eb.sig_item.len() == 1 && eb.sig_item[0].is_empty(),
        "real Submit item empty"
    );
    ...
    let ec = send(
        &mut c,
        &event(dispatch::E_KEY_DOWN, "Enter", "", dispatch::M_SHIFT),
    );
    assert_eq!(
        edit::text_str(&c.ds.ed[0]),
        "a\n",
        "Shift+Enter inserts with submit binding"
    );
    assert!(
        ec.sig_name.len() == 1 && ec.sig_name[0] == 1,
        "modified Enter emits Change only"
    );
    ...
    let eca = send(
        &mut ca,
        &event(dispatch::E_KEY_DOWN, "Enter", "", dispatch::M_ALT),
    );
    assert_eq!(
        edit::text_str(&ca.ds.ed[0]),
        "a\n",
        "Alt+Enter inserts with submit binding"
    );
    assert!(
        eca.sig_name.len() == 1 && eca.sig_name[0] == 1,
        "Alt+Enter emits Change only"
    );
    ...
    assert!(
        ed.sig_name.len() == 1 && ed.sig_name[0] == 2,
        "single-line submit fires"
    );
...
pub fn test_single_line_text_prefilters_newlines() {
    ...
    assert_eq!(
        edit::text_str(&f.ds.ed[0]),
        "a b",
        "single line maps newline to space"
    );
}
...
pub fn test_visual_arrows_home_end_and_caret_geometry() {
    ...
    assert_eq!(
        f.ds.ed[0].caret, 2,
        "Up preserves goal x and reaches prior visual line end"
    );
    ...
    assert!(
        up.has_ime && up.ime_x == up.caret_x && up.ime_y == up.caret_y,
        "IME rect equals caret"
    );
    ...
    assert_eq!(
        f.ds.ed[0].caret, 3,
        "Down reaches next visual line at goal x"
    );
...
pub fn test_caret_geometry_honors_padding_and_alignment() {
    ...
    assert!(
        (eff.caret_x - 81.6).abs() < 0.01,
        "right-aligned padded caret x"
    );
...
pub fn test_kills_undo_and_redo() {
    ...
    send(
        &mut f,
        &event(dispatch::E_KEY_DOWN, "Delete", "", dispatch::M_CTRL),
    );
    assert_eq!(
        edit::text_str(&f.ds.ed[0]),
        "cd",
        "Ctrl+Delete removes word forward"
    );
    send(
        &mut f,
        &event(dispatch::E_KEY_DOWN, "z", "", dispatch::M_CTRL),
    );
    assert_eq!(
        edit::text_str(&f.ds.ed[0]),
        "ab cd",
        "undo restores deletion"
    );
    send(
        &mut f,
        &event(
            ...
            "Z",
            "",
            dispatch::M_CTRL | dispatch::M_SHIFT,
        ),
    );
    ...
    send(
        &mut k,
        &event(dispatch::E_KEY_DOWN, "u", "", dispatch::M_CTRL),
    );
    assert_eq!(
        edit::text_str(&k.ds.ed[0]),
        "ab\n",
    ...
    send(
        &mut k,
        &event(dispatch::E_KEY_DOWN, "k", "", dispatch::M_CTRL),
    );
    assert_eq!(
        edit::text_str(&k.ds.ed[0]),
        "ab\n",
...
pub fn test_paste_undoes_in_one_step() {
    ...
    assert_eq!(
        edit::text_str(&f.ds.ed[0]),
        "abone\ntwo",
        "paste inserts both lines wholesale"
    );
    send(
        &mut f,
        &event(dispatch::E_KEY_DOWN, "z", "", dispatch::M_CTRL),
    ...
    assert_eq!(
        edit::text_str(&f.ds.ed[0]),
        "ab",
        "one undo restores the pre-paste text"
    );
}
...
pub fn test_horizontal_and_ancestor_scroll_follow() {
    ...
    assert!(
        f.ds.ed[0].scroll_x > 0.0,
        "narrow single line follows caret horizontally"
    );
    assert_eq!(
    ...
    assert!(
        style::scroll_get(&g.st, 1) > 0.0,
        "nearest scroll ancestor follows multiline caret"
    );
}
...
pub fn test_fresh_wrapped_layout_scroll_follow_settles() {
    ...
    assert!(
        style::scroll_get(&f.st, 1) > 0.0,
        "fresh wrapped caret scrolls ancestor"
    );
}
