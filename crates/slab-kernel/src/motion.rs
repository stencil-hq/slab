//! Motion overlays interpolate animation inputs before each normal layout
//! solve.
//!
//! Each intermediate frame is solved like any other document, so containment
//! and other layout invariants hold throughout an animation. Transitions use
//! one kernel-managed clock per state patch: a state flip records the solve
//! time, and the patch value is blended with its base while `t - flip - delay <
//! duration`. Entering uses the eased progress as its weight; leaving uses its
//! complement. Attributes without a base step at the midpoint. Flags and
//! conditional children are always discrete.
...
pub const fn rgba_swap(c: u32) -> u32 {
...
pub fn lerp_v(d: &Doc, st: &mut St, a: &V, b: &V, f: f64) -> V {
	...
	if is_tuplelike(a.tag) && is_tuplelike(b.tag) && a.ln == b.ln {
		...
		for k in 0..a.ln {
			...
			st.mo_f.push((bv - av).mul_add(f, av));
		}
...
pub fn keyframe_v(d: &Doc, st: &mut St, anim: i32, attr: u32, p: f64) -> V {
	...
	previous.map_or_else(value::missing, |(_, value_ix)| pv(d, st, &value::decode(d, value_ix)))
}
...
/// Returns the last value index for an attribute in a patch, or `-1` when
/// absent.
...
pub fn sp_find(ms: &MSt, node: u32, pi: i32) -> i32 {
	...
	ms.sp_slot
		.get(&(node, pi))
		.map_or(-1, |&clock| i32::try_from(clock).expect("synthetic clock index exceeds i32"))
}
...
const fn vend_of(resolved: &V) -> Option<VEnd> {
	if is_numlike(resolved.tag) || is_pctlike(resolved.tag) {
		return Some(VEnd { tag: resolved.tag, num: resolved.num, rgba: 0 });
	}
	if is_colorlike(resolved.tag) {
		return Some(VEnd { tag: resolved.tag, num: 0.0, rgba: resolved.h });
	}
	None
}
...
fn vend_lerp(from: VEnd, to: VEnd, weight: f64) -> VEnd {
	...
	if is_colorlike(from.tag) && is_colorlike(to.tag) {
		return VEnd { tag: to.tag, num: 0.0, rgba: lerp_rgba(from.rgba, to.rgba, weight) };
	}
	VEnd { tag: to.tag, num: (to.num - from.num).mul_add(weight, from.num), rgba: 0 }
}
...
fn active_patch_writes(d: &Doc, st: &St, node: u32, attr: u32) -> bool {
	...
	st.lists.patches_by_node[node_index].iter().any(|&patch| {
		st.patch_on[patch]
			&& patch_attr_ix(d, i32::try_from(patch).expect("patch index exceeds i32"), attr) >= 0
	})
}
	...
	for transition in 0..d.trans_node.len() {
		...
		for entry in lo..hi {
			...
			let slot = if let Some(&slot) = ms.v_slot.get(&(node, attr)) {
				...
				ms.v_clocks
					.push(VClock { node, attr, last: now, from: now, flip: NEVER });
				slot
			};
			...
			if now != clock.last {
				...
				clock.from = if clock.flip > NEVER && age < dur {
					vend_lerp(clock.from, clock.last, ease_code(easing, 0.0f64.max(age) / dur))
				} else {
					clock.last
				};
				clock.last = now;
...
pub fn apply(d: &Doc, st: &mut St, ms: &mut MSt, t: f64) -> bool {
	...
	for transition in 0..d.trans_node.len() {
		...
		for &attr in &attrs {
			...
			for position in 0..patch_count {
				let patch = st.lists.patches_by_node[node_index][position];
				let target_ix =
					patch_attr_ix(d, i32::try_from(patch).expect("patch index exceeds i32"), attr);
				if target_ix < 0 {
					continue;
				}
				let target = pv(d, st, &value::decode(d, target_ix));
				...
				if in_flight {
					...
					// Other values without a base have nothing to blend from and
					// step at 50%.
	...
	// Template transitions need distinct clocks and overlays per synthetic
	// instance.
	let mut attrs = std::mem::take(&mut ms.scratch_attrs);
	for transition in 0..d.trans_node.len() {
...
///   render-only containers, `rotate`/`scale` on `rect`/`image`/`path`, solid
///   `bg` on plain rects and paths, and text `color`;
/// - every keyframe (and each base used for a delta) is a static literal, so no
...
fn segment_ctrl(code: u32, t0: f64, t1: f64, p0: f64, p1: f64) -> (f64, f64) {
	...
	let b = (2.0 * alpha).mul_add(t0, beta) * span / progress;
...
fn css_lerp_rgba(a: u32, b: u32, f: f64) -> u32 {
	...
	let a1 = ch(a, 24);
	let a2 = ch(b, 24);
	let am = (a2 - a1).mul_add(f, a1);
	let mix = |shift: u32| -> u32 {
		let v = if am > 0.0 {
			(ch(b, shift) * a2).mul_add(f, ch(a, shift) * a1 * (1.0 - f)) / am
		} else {
			(ch(b, shift) - ch(a, shift)).mul_add(f, ch(a, shift))
		};
...
const fn lift_leaf(kind: u32) -> bool {
	matches!(
		kind,
		slir::K_TEXT
			| slir::K_RECT
...
const fn lift_container(kind: u32) -> bool {
	matches!(
		kind,
		slir::K_ROW
			| slir::K_COL
...
fn lift_of(d: &Doc, binding: usize) -> Option<Lift> {
	...
	let animation = index(i32::try_from(d.bind_anim[binding]).expect("animation index exceeds i32"));
	let stop_start = d.anim_stop_off[animation];
	let stop_end = stop_start.wrapping_add(d.anim_stop_len[animation]);
	...
	if attrs.contains(&slir::A_SCALE) {
		...
		base_scale = match base_sc.tag {
			...
			slir::T_TUPLE if base_sc.ln >= 2 => {
				(value::tuple_at(d, &base_sc, 0), value::tuple_at(d, &base_sc, 1))
			},
			_ => return None,
		};
	...
	let stops = positions
		...
		.map(|(i, (&pos, &t))| LiftStop {
			...
			opacity: has(slir::A_OPACITY).then(|| track_num(d, animation, slir::A_OPACITY, pos, -1)),
...
fn track_num(d: &Doc, animation: usize, attr: u32, pos: f64, component: i32) -> f64 {
	let read = |aval: u32| -> f64 {
		let v = value::decode(d, i32::try_from(aval).expect("attribute value index exceeds i32"));
		if component < 0 {
			value::num_of(&v, 0.0)
		} else {
	...
	for stop in stop_start..stop_end {
		...
		for entry in attr_start..attr_end {
			...
			if pos <= current.0 {
				...
				return (current.1 - previous_num).mul_add(f, previous_num);
			}
			previous = Some(current);
		}
	}
	previous.map_or(0.0, |(_, num)| num)
}
...
fn track_rgba(d: &Doc, animation: usize, attr: u32, pos: f64) -> u32 {
	let read = |aval: u32| -> u32 {
		value::decode(d, i32::try_from(aval).expect("attribute value index exceeds i32")).h
	};
