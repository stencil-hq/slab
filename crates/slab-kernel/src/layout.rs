...
pub fn resolve_len(
	...
	match kind {
		...
		crate::style::S_PCT if has_pct => Own { has: true, v: (v / 100.0) * pct_base },
		crate::style::S_PCT => {
			crate::style::warn(
				st,
				"pct-unbounded",
				"% against an unbounded or hug axis behaves as hug (give the parent a determinate \
				 size or use fill)",
				line,
			);
			Own { has: false, v: 0.0 }
...
/// Wraps a rotation angle into `[0, 360)` without changing floating-point
/// semantics.
pub fn wrap360(v: f64) -> f64 {
	(v / 360.0).floor().mul_add(-360.0, v)
}
...
/// Marks overflowing placements as clipped unless bleeding was explicitly
/// allowed.
///
...
/// Resolves the overlay for `kids[index]` when the following child is a
/// divider.
///
/// The next pane's authored minimum and the divider's own footprint are
/// reserved from the current box budget. Pointer drags impose the stronger
/// gesture-start snapshot bound before they write the same overlay.
...
pub fn box_measure(d: &Doc, st: &mut St, l: &mut Lay, node: u32, ri: i32, cn: &Cons) -> i32 {
	...
	if !has_cross {
		...
		own_cross = clamp(needed + pad_cross, cmin_cross, cmax_cross, amin_cross, amax_cross);
	}
	...
	for ln in 0i32..(nlines) {
		...
		for i in 0i32..(len_i32(&kp)) {
			if kline[idx(i)] == ln {
				let ci = kp[idx(i)];
				l.p_x[idx(ci)] = x;
				l.p_y[idx(ci)] = af.mul_add(line_h - l.p_h[idx(ci)], y);
...
pub fn grid_measure(d: &Doc, st: &mut St, l: &mut Lay, node: u32, ri: i32, cn: &Cons) -> i32 {
		...
		if tk[idx(t)] == 0u32 {
			...
			if w < (tv[idx(t)] - EPS) {
				let msg = crate::rt::str_concat(
					...
					&crate::rt::str_concat(&crate::rt::str_concat(", got ", &crate::value::fmt3(w)), ""),
				);
	...
	for i in 0i32..(len_i32(&kids).wrapping_add(1i32)) {
		...
		if flushrow && (i > row_start) {
			...
			for k in row_start..(i) {
				...
				let sh =
					crate::style::peek_size(d, st, kids[idx(k)], false, crate::slir::K_GRID, is_row);
	...
	if !has_w {
		...
		own_w =
			clamp((sumw + pl) + pr, cn.min_w, cn.max_w, st.rs[idx(ri)].min_w, st.rs[idx(ri)].max_w);
	}
	let oh = resolve_len(
		st,
		st.rs[idx(ri)].h_kind,
...
pub fn stack_measure(d: &Doc, st: &mut St, l: &mut Lay, node: u32, ri: i32, cn: &Cons) -> i32 {
	...
	if !has_w {
		...
		own_w = clamp((mw + pl) + pr, cn.min_w, cn.max_w, st.rs[idx(ri)].min_w, st.rs[idx(ri)].max_w);
	}
	if !has_h {
		let mh = overlay_hug_extent(st, l, &kp, false, false);
		own_h = clamp((mh + pt) + pb, cn.min_h, cn.max_h, st.rs[idx(ri)].min_h, st.rs[idx(ri)].max_h);
	}
	let cw = (own_w - pl) - pr;
	let ch = (own_h - pt) - pb;
	let pi = p_new(l, node, ri);
	...
	for k in 0i32..(len_i32(&kp)) {
		...
		l.p_x[idx(ci)] =
			(cw - l.p_w[idx(ci)]).mul_add(crate::style::nine_fx(a), pl) + st.rs[idx(cri)].offset_x;
		l.p_y[idx(ci)] =
			(ch - l.p_h[idx(ci)]).mul_add(crate::style::nine_fy(a), pt) + st.rs[idx(cri)].offset_y;
		l.child_pool.push(ci);
	}
	l.p_child_len[idx(pi)] = len_i32(&l.child_pool).wrapping_sub(l.p_child_off[idx(pi)]);
	l.p_w[idx(pi)] = own_w;
...
pub fn canvas_measure(d: &Doc, st: &mut St, l: &mut Lay, node: u32, ri: i32, cn: &Cons) -> i32 {
	...
	if !has_w {
		...
		own_w = clamp(mx + pr, cn.min_w, cn.max_w, st.rs[idx(ri)].min_w, st.rs[idx(ri)].max_w);
	}
	if !has_h {
		let my = overlay_hug_extent(st, l, &kp, false, true);
		own_h = clamp(my + pb, cn.min_h, cn.max_h, st.rs[idx(ri)].min_h, st.rs[idx(ri)].max_h);
	}
	let pi = p_new(l, node, ri);
	l.p_child_off[idx(pi)] = len_i32(&l.child_pool);
	for k in 0i32..(len_i32(&kp)) {
...
pub fn para_measure(d: &Doc, st: &mut St, l: &mut Lay, node: u32, ri: i32, cn: &Cons) -> i32 {
	...
	for i in 0i32..(len_i32(&w_a)) {
		...
		let mut add = (f64::from(eff)).mul_add(sp, ww);
...
pub fn img_measure(d: &Doc, st: &mut St, l: &mut Lay, node: u32, ri: i32, cn: &Cons) -> i32 {
	...
	let (natural_w, natural_h) = dimensions
		.filter(|&(w, h)| w > 0 && h > 0)
		.map_or((64.0, 64.0), |(w, h)| (f64::from(w), f64::from(h)));
	let mut own_w = ow.v;
	let mut own_h = oh.v;
	if !ow.has && !oh.has {
		own_w = natural_w;
...
pub fn path_measure(d: &Doc, st: &mut St, l: &mut Lay, node: u32, ri: i32, cn: &Cons) -> i32 {
	...
	let (natural_w, natural_h) = crate::style::path_coords(d, st, path)
		...
		.map_or((0.0, 0.0), |(min_x, min_y, max_x, max_y)| (max_x - min_x, max_y - min_y));
	let line = st.rs[idx(ri)].line;
...
const fn opposite_gravity(gravity: crate::style::Gravity) -> crate::style::Gravity {
	use crate::style::Gravity;
	match gravity {
...
const fn vertical_gravity(gravity: crate::style::Gravity) -> bool {
	matches!(
		gravity,
...
const fn positive_gravity(gravity: crate::style::Gravity) -> bool {
	matches!(
		gravity,
		crate::style::Gravity::BelowStart
			| crate::style::Gravity::BelowCenter
...
const fn flipped_alignment(gravity: crate::style::Gravity) -> crate::style::Gravity {
	use crate::style::Gravity;
	match gravity {
...
fn collect_placements(
	...
	if l.p_rot[placement] >= 0 {
		collect_placements(l, l.p_rot[placement], pi, true, -1, parents, rotated, slots, actual);
		return;
	}
...
impl Affine {
	...
	fn rotation(cx: f64, cy: f64, deg: f64) -> Self {
		...
		Self {
			...
			c:  -sin,
			d:  cos,
			tx: sin.mul_add(cy, cos.mul_add(-cx, cx)),
			ty: cos.mul_add(-cy, sin.mul_add(-cx, cy)),
		}
	}
	...
	fn compose(self, inner: Self) -> Self {
		Self {
			a:  self.c.mul_add(inner.b, self.a * inner.a),
			b:  self.d.mul_add(inner.b, self.b * inner.a),
			c:  self.c.mul_add(inner.d, self.a * inner.c),
			d:  self.d.mul_add(inner.d, self.b * inner.c),
			tx: self.c.mul_add(inner.ty, self.a * inner.tx) + self.tx,
			ty: self.d.mul_add(inner.ty, self.b * inner.tx) + self.ty,
		}
	}
	...
	fn inverse_map(self, x: f64, y: f64) -> (f64, f64) {
		...
		let x = x - self.tx;
		let y = y - self.ty;
		(self.c.mul_add(-y, self.d * x) / det, self.a.mul_add(y, -self.b * x) / det)
	}
}
...
fn painted_rect(transform: Affine, x: f64, y: f64, w: f64, h: f64) -> AttachRect {
	...
	AttachRect { x: min_x, y: min_y, w: max_x - min_x, h: max_y - min_y }
}
		...
		if !self.rotated[placement] {
			...
			if rule.has_attach {
				...
				let (painted_x, painted_y) = if self.st.env.client == crate::when::CLIENT_TUI {
					(
						f64::from(crate::cells::rhe(painted_x / crate::cells::CW)) * crate::cells::CW,
						f64::from(crate::cells::rhe(painted_y / crate::cells::CH)) * crate::cells::CH,
					)
				} else {
					(painted_x, painted_y)
				};
...
pub fn solve(d: &Doc, st: &mut St, l: &mut Lay, width: f64, height: f64, has_height: bool) -> i32 {
	...
	measure(d, st, l, 0, &constraints, 255, false, &root_inh, false, false, false)
}
...
mod attachment_tests {
	...
	fn twelve_gravities_place_on_the_expected_edge() {
		use style::Gravity::*;
		let anchor = AttachRect { x: 100.0, y: 80.0, w: 40.0, h: 20.0 };
		let cases = [
		...
		for (gravity, expected) in cases {
			assert_eq!(gravity_position(anchor, 30.0, 10.0, gravity), expected, "{gravity:?}");
		}
	}

	#[test]
	fn collision_flips_main_then_alignment_before_offset() {
		let anchor = AttachRect { x: 140.0, y: 105.0, w: 10.0, h: 10.0 };
	...
	fn alignment_flip_precedes_slide_and_falls_back_when_both_overflow() {
		...
		let chip = AttachRect { x: 48.0, y: 40.0, w: 62.0, h: 24.0 };
		assert_eq!(
			attachment_position(
				chip,
				150.0,
		...
		let wide = AttachRect { x: 10.0, y: 10.0, w: 30.0, h: 10.0 };
		assert_eq!(
			attachment_position(
				wide,
				80.0,
	...
	fn rotated_bounds_and_inverse_parent_mapping_share_paint_geometry() {
		...
		for (actual, expected) in
			[(anchor.x, 45.0), (anchor.y, 40.0), (anchor.w, 10.0), (anchor.h, 20.0)]
		{
		...
		for (actual, expected) in [(popup.x, 30.0), (popup.y, 10.0), (popup.w, 10.0), (popup.h, 30.0)]
		{
			assert!((actual - expected).abs() < 1e-9, "{actual} != {expected}");
		}
	}
	...
	fn attachment_fixture(scroll: f64) -> (Doc, St, Lay, usize) {
		...
		d.node_kind
			.extend([slir::K_STACK, slir::K_COL, slir::K_RECT, slir::K_RECT, slir::K_RECT]);
