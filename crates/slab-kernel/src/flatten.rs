...
pub struct SceneNode {
	...
	pub is_row:         bool,
	pub src_line:       u32,
	/// Pre-order rank in the materialized authored tree, independent of paint
	/// promotion.
...
const fn clip(x: f64, y: f64, w: f64, h: f64, radius: f64, smooth: f64) -> OpClip {
	OpClip { x, y, w, h, radius, smooth }
}

const fn unstyled_rect(node: u32, x: f64, y: f64, w: f64, h: f64, radius: f64) -> OpRect {
	OpRect {
		node,
		x,
		y,
...
const fn text_alignment_factor(alignment: u32) -> f64 {
	match alignment {
		1 => 0.5,
		2 => 1.0,
		_ => 0.0,
...
fn frame_path_ref(d: &slir::Doc, st: &St, fr: &mut Frame, path: i32) -> Option<i32> {
	...
	let mut pooled = fr
		.path_pool
		.pop()
		.unwrap_or_else(|| RtPath { verbs: Vec::new(), coords: Vec::new() });
	pooled.verbs.clear();
...
fn emit_icon(
	...
	fr.ops
		.push(FrameOp::ScalePush(OpScale { cx: x, cy: y, sx: scale, sy: scale }));
	...
	while child != slir::NONE {
		...
		if d.node_kind[child_index] == slir::K_PATH {
			let path_value =
				crate::value::decode_active(d, st.theme_index, slir::base_attr(d, child, slir::A_D));
			if path_value.tag == slir::T_PATH_REF {
				let (bg_kind, bg) = icon_paint(d, st, child, slir::A_BG, rule.color_kind, rule.color);
...
fn push_scrollbar_axis(
	...
	push_solid_rect(fr, node, track_x, track_y, track_width, track_height, rule.scrollbar_bg);
...
fn push_uncovered_runs(d: &slir::Doc, fr: &mut Frame, font: i32, str_ref: i32) -> (i32, i32) {
	...
	let Frame { strings, uncovered, .. } = fr;
...
pub fn walk(
	...
	let rotation = has_rot.then_some(RotationFrame { deg: in_deg, cx: in_cx, cy: in_cy });
...
fn walk_node(
	...
	if l.p_rot[pi] >= 0 {
		...
		let rotation = RotationFrame { deg: rule.rotate, cx: x + w / 2.0, cy: y + h / 2.0 };
	...
	for child_pool_index in first_child..child_end {
		...
		walk_node(d, st, l, ds, ms, fr, child_pi, sticky_x, sticky_y, authored_order, WalkContext {
			parent_ix:    scene_index_i32,
			parent_inert: inert,
			rotation:     None,
			range:        context.range,
...
pub fn flatten_into(
	...
	walk(d, st, l, ds, ms, frame, root_pi, 0.0, 0.0, -1, false, false, 0.0, 0.0, 0.0);
