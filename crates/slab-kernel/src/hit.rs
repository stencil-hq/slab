...
pub fn contains(scene: &Scene, node: i32, x: f64, y: f64) -> bool {
    ...
    for link in chain {
        ...
        let rotation = scene.rot[index];
        if rotation != 0.0 {
            let local_x = unrotate_x(px, py, rotation, scene.cx[index], scene.cy[index]);
            let local_y = unrotate_y(px, py, rotation, scene.cx[index], scene.cy[index]);
            px = local_x;
            py = local_y;
        }
        ...
        if scene.flags[index] & F_CLIP != 0 && !inside {
            return false;
        }
    }
...
pub fn hit_test(scene: &Scene, x: f64, y: f64, out: &mut Vec<i32>) {
    ...
    for offset in 0..node_count {
        ...
        if scene.flags[index] & F_INERT != 0 {
            continue;
        }
