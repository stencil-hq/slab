...
pub fn rgba_lerp(a: u32, b: u32, t: f64) -> u32 {
	...
	for shift in [0, 8, 16, 24] {
		...
		let channel = truncate_i32((to - from).mul_add(t, from).round());
...
pub fn mask_alpha_at(doc: &Doc, masks: &[MaskCtx], px: f64, py: f64) -> f64 {
	...
	for mask in masks {
		...
		let rgba = paint_rgba_at(doc, mask.kind, mask.handle, px, py, mask.x, mask.y, mask.w, mask.h);
...
pub fn draw_rect(
	...
	if rect.h <= CH * 0.6 && rect.w > CW {
		...
		if alpha_of(
			paint_rgba_at(doc, paint_kind, paint, rect.x, rect.y, rect.x, rect.y, rect.w, rect.h),
			effective_opacity,
		) < 8
		{
		...
		for column in left..right {
			...
			let rgba =
				paint_rgba_at(doc, paint_kind, paint, point_x, rect.y, rect.x, rect.y, rect.w, rect.h);
			let masked =
	...
	if rect.w <= CW * 0.9 && rect.h > CH {
		...
		for row in top..bottom {
			...
			let rgba =
				paint_rgba_at(doc, paint_kind, paint, rect.x, point_y, rect.x, rect.y, rect.w, rect.h);
			let masked =
				effective_opacity * mask_alpha_at(doc, masks, (f64::from(column) + 0.5) * CW, point_y);
			let foreground = stroke_fg(grid, column, row, rgba, masked);
			if foreground != NO_COLOR {
				put(grid, column, row, ch, foreground, NO_COLOR);
			}
		...
		for row in top..bottom {
			for column in left..right {
				...
				bg_composite(
					...
					row,
					rgb_of(rgba),
					alpha_of(rgba, effective_opacity * mask_alpha_at(doc, masks, point_x, point_y)),
				);
			}
		}
	}
...
fn draw_rect_outline(
	...
	if right.wrapping_sub(left) < 2 || bottom.wrapping_sub(top) < 2 {
				...
				.any(|code| code.as_str() == "stroke-band")
		{
			note(
				grid,
				"stroke-band",
				"tui stroke outline needs two fully covered cell rows and columns; border suppressed \
				 (align the node to the 8x16 cell grid, give it more room, or use bg cues)",
			);
		}
		return;
	}
	...
	if corner_fg != NO_COLOR {
		...
		if claims(grid, claim, last_column, last_row) {
			put(grid, last_column, last_row, bottom_right, corner_fg, NO_COLOR);
		}
	}
}
		...
		match verb {
			...
			2 | 3 => {
				...
				let (control_x2, control_y2, end_x, end_y, consumed) = if verb == 2 {
					...
					(control_x1, control_y1, coords[coordinate + 2] + dx, coords[coordinate + 3] + dy, 4)
				};
				coordinate += consumed;
				for step in 1..9 {
					...
					let (x, y) = if verb == 2 {
						...
						(
							(t * t).mul_add(end_x, (2.0 * u * t).mul_add(control_x1, u * u * x0)),
							(t * t).mul_add(end_y, (2.0 * u * t).mul_add(control_y1, u * u * y0)),
						)
					};
					xs.push(x);
					ys.push(y);
...
/// Degrades a path into an even-odd cell-center fill and slope-character
/// strokes.
///
...
pub fn draw_path(
	...
	if path.path < 0 {
		...
		path_polylines(doc, path.path, path.dx, path.dy, &mut xs, &mut ys, &mut starts);
	}
	if xs.len() < 2 {
		return;
	}
	...
	for (subpath_index, start) in starts.iter().enumerate() {
		...
		for edge in start..end.saturating_sub(1) {
			...
			for step in 0..steps.wrapping_add(1) {
				...
				let point_x = (x2 - x1).mul_add(t, x1);
				let point_y = (y2 - y1).mul_add(t, y1);
		...
		match operation {
			...
			flatten::FrameOp::Rect(rect) if rotation_depth == 0 => {
				if rect.grain_amount > 0.0 && !warned_grain {
					note(&mut grid, "cap-grain", "tui renderer cannot speckle; grain overlay skipped");
					warned_grain = true;
				}
				draw_rect(
					doc,
...
pub const fn is_trailing_ws(codepoint: u32) -> bool {
	...
	for row in 0..grid.rows {
		...
		if plain {
			for column in 0..grid.cols {
				push_cell_text(&mut line, grid, row.wrapping_mul(grid.cols).wrapping_add(column));
			}
		} else {
...
mod attrs_tests {
	...
	fn attrs_plane_reports_strike_and_color_runs() {
		...
		g.fg[9] = 0x00e9_eef6;
		g.bg[9] = 0x0016_1b24;
		assert_eq!(cells_attrs_text(&g), "row 000: 2-4 strike\nrow 001: 1-1 fg=#E9EEF6 bg=#161B24\n");
	}
	...
	fn attrs_plane_splits_runs_on_any_attribute_change() {
		...
		assert_eq!(cells_attrs_text(&g), "row 000: 0-1 fg=#FF0000; 2-3 fg=#00FF00\n");
	}
