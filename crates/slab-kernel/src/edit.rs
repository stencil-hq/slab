...
pub fn display_str(es: &EditState) -> String {
    ...
    display.push_str(&crate::rt::str_slice(
        &es.text,
        es.caret,
        crate::rt::str_len(&es.text),
    ));
    display
}
...
pub fn history_barrier(es: &mut EditState) {
...
pub fn splice_out(es: &mut EditState, lo: i32, hi: i32) {
    ...
    text.push_str(&crate::rt::str_slice(
        &es.text,
        hi,
        crate::rt::str_len(&es.text),
    ));
...
pub fn insert(es: &mut EditState, text: &str) -> bool {
    ...
    if added > 0 {
        ...
        committed.push_str(&crate::rt::str_slice(
            &es.text,
            es.caret,
            crate::rt::str_len(&es.text),
        ));
...
pub fn home(es: &mut EditState, select: bool) {
    history_barrier(es);
    es.goal_x = NO_GOAL_X;
    es.caret = 0;
    if !select {
...
pub fn visual_move(
    ...
    if es.goal_x < 0.0 {
        ...
        es.goal_x = textm::str_slice_w(
            d,
            font,
            size,
            tracking,
            &es.text,
            tl.src_ls[line_index],
            es.caret,
        );
    }
