...
impl K {
	const fn is_closer(self) -> bool {
		matches!(self, Self::Rb | Self::Rp | Self::Rs)
	}
}
		...
		match c {
			...
			b'\\'
				if matches!(b.get(i + 1), Some(b'\n'))
					|| (matches!(b.get(i + 1), Some(b'\r')) && matches!(b.get(i + 2), Some(b'\n'))) =>
			{
				out.push(S::Cont);
...
fn assemble(toks: Vec<S>) -> Vec<Line> {
	...
	let flush = |cur: &mut Vec<Part>,
		...
		lines.push(Line::Text { indent, parts: std::mem::take(cur), ctx: line_ctx, cont });
	};
	...
	for t in toks {
		match t {
			S::Nl | S::Cont => {
				...
				flush(&mut cur, &mut lines, line_depth, line_ctx, cont, &mut cont_base);
				if !cont {
					first_word = None;
				}
				line_depth = depth;
			...
			S::Tok(k, text) => {
				...
				let sep = if cur.is_empty() {
					...
					sep(cur.last().unwrap().k, k, paren, *ctxs.last().unwrap_or(&Ctx::Node))
				};
				match k {
					K::Lb => {
						depth += 1;
						ctxs.push(block_ctx(first_word.as_deref(), *ctxs.last().unwrap_or(&Ctx::Node)));
					},
					K::Rb => {
	...
	flush(&mut cur, &mut lines, line_depth, line_ctx, false, &mut cont_base);
	lines
}
...
fn align_width(l: &Line) -> Option<usize> {
	let Line::Text { parts, ctx, cont, .. } = l else {
		return None;
	};
...
fn render(lines: Vec<Line>) -> String {
	...
	let mut any = false;
	for (i, l) in lines.iter().enumerate() {
		let Line::Text { indent, parts, cont, .. } = l else {
			pending_blank = any;
			continue;
		};
		if pending_blank {
...
mod tests {
	...
	fn aligns_anim_keyframes() {
		let src = "anim spin {\n0% {rotate=0}\n100% {rotate=360}\n}\n";
		assert_eq!(format(src), "anim spin {\n  0%   { rotate=0 }\n  100% { rotate=360 }\n}\n");
	}

	#[test]
	fn commas_tight_in_tuples_spaced_in_calls() {
		let src = "rect pad=6 , 12 bg=linear(90 ,#fff 0% ,#000 100%)\n";
		assert_eq!(format(src), "rect pad=6,12 bg=linear(90, #fff 0%, #000 100%)\n");
	}
