...
use crate::lex::{Tok, TokKind, lex, py_repr};
...
impl<'d> Parser<'d> {
    ...
    pub fn parse(&mut self) -> Document {
        ...
        while !self.at(TokKind::Eof) {
            ...
            if t.kind == TokKind::Id && t.text == "tokens" {
                ...
                self.diags.error(
                    "parse",
                    "`each` is only allowed inside a node block",
                    t.line,
                );
                self.parse_each();
    ...
    fn parse_def(&mut self) -> ADef {
        ...
        if !name.chars().next().is_some_and(|c| c.is_uppercase()) {
            self.diags.error(
                "parse",
                format!("component names must be Capitalized: {name}"),
                line,
            );
        }
        self.expect(TokKind::Lp, "'('");
    ...
    fn parse_icon(&mut self) -> AIcon {
        ...
        while !matches!(
            self.peek().kind,
            TokKind::Nl | TokKind::Lb | TokKind::Rb | TokKind::Eof
        ) {
            ...
            if attr.text == "viewbox" {
                ...
                self.diags.error(
                    "parse",
                    format!("unknown icon attribute '{}'", attr.text),
                    attr.line,
                );
            }
        }
...
"#;
    ...
    fn parses_cross_scroll_modes_and_sticky_flag() {
        let mut diags = Diagnostics::default();
        let doc = parse(
            "col scroll=both { row scroll=cross { rect sticky } }\n",
            &mut diags,
        );
        assert!(diags.0.is_empty(), "{:?}", diags.0);
        assert!(matches!(
