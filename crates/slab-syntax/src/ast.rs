...
pub struct AEach {
	...
	pub param: String,
	/// True when the target is an enclosing item property rather than a root
	/// param.
	pub prop:  bool,
...
impl TokenTree {
	...
	pub fn deep_merge(&mut self, src: &Self) -> Vec<String> {
		let mut dups = Vec::new();
		self.merge_inner(src, &mut String::new(), &mut dups);
		dups
	}

	fn merge_inner(&mut self, src: &Self, prefix: &mut String, dups: &mut Vec<String>) {
		for (k, v) in &src.0 {
