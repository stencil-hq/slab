...
pub fn lex(src: &str, diags: &mut Diagnostics) -> Vec<Tok> {
	...
	toks.push(Tok { kind: TokKind::Eof, text: String::new(), line: map.line(src.len()), val: 0.0 });
	toks
}

fn simple(kind: TokKind, text: &str, line: u32) -> Tok {
	Tok { kind, text: text.into(), line, val: 0.0 }
}
...
mod tests {
	...
	fn lexes_list_brackets() {
		...
		assert_eq!(toks.iter().map(|tok| tok.kind).collect::<Vec<_>>(), vec![
			TokKind::Ls,
			TokKind::Rs,
			TokKind::Eof
		]);
