...
//!   (inserting a `## Platform support` section before the changelog when the
//!   markers are absent). Idempotent.
...
fn parse_support(text: &str) -> Result<Vec<Feature>, String> {
	...
	for (ln, raw) in text.lines().enumerate() {
			...
			.and_then(|s| s.strip_suffix(']'))
		{
			...
			feats.push(Feature { name: name.to_string(), cells: Vec::new() });
			continue;
		}
		...
		let cell = if val == "full" {
			...
			return Err(err(&format!("bad value '{val}' (full | degraded:<note> | none:<cap-code>)")));
		};
...
fn chart_md(feats: &[Feature]) -> String {
	...
	md.push('\n');
	md.push_str(
		"<!-- GENERATED from spec/support.toml by `cargo run -p xtask -- support-md`; edit the \
		 toml, not this table. -->\n\n",
	);
	md.push_str("| feature |");
	for c in CLIENTS {
...
fn section_md(feats: &[Feature]) -> String {
	format!(
		"## Platform support\n\nOne row per feature, one column per renderer client (§11's \
		 `when`\nclient classes; the column order is the kernel client code). **full**\n= renders \
		 as specified. **degraded** = renders with the documented\napproximation; the note is the \
		 whole contract. **none** = the\nrenderer skips the feature and reports the `cap-*` code \
		 once per\ndocument (§12 notes; TUI notes accumulate on the cell grid, static\nexporters \
		 print to stderr). Machine-readable source:\n`spec/support.toml`; the driver lookup table \
		 is generated into\n`crates/slab-kernel/src/caps.rs` by `cargo run -p xtask -- \
		 gen-caps`.\n\n{}\n",
		chart_md(feats)
	)
}
