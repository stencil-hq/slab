...
fn attr_label(id: u16) -> String {
	attrs::attr_name(id).map_or_else(|| format!("attr{id}"), str::to_string)
}
...
pub fn dump(s: &Slir) -> String {
	...
	let _ = writeln!(o, "AVAL {} f64s={}{tup_dyn_note}", s.avals.len(), s.f64s.len());
	...
	for (i, c) in s.conds.iter().enumerate() {
		...
		let body = match c.kind {
			cond::WCMP | cond::HCMP => {
				format!("{} {}", cond::OPS.get(c.op as usize).copied().unwrap_or("?"), fmt_f(c.num))
			},
	...
	for (i, a) in s.anims.iter().enumerate() {
		let stops: Vec<String> = a
			...
			.collect();
		let _ = writeln!(o, "  anim {i} name={:?} stops {}", s.str_at(a.name), stops.join(" "));
	}
	...
	for (i, list) in s.lists.iter().enumerate() {
		let _ =
			writeln!(o, "  {i} param={} fields=@{}+{}", list.param, list.field_off, list.field_len);
	}
	...
	for (i, &(name, node, trigger)) in s.signals.iter().enumerate() {
		let _ =
			writeln!(o, "  {i} name={:?} node={node} trigger={}", s.str_at(name), match trigger {
				0 => "activate",
				1 => "change",
				2 => "submit",
				3 => "press",
	...
	for (index, token) in s.tokens.iter().enumerate() {
		...
		for &(theme, value, repr) in &token.themes {
			let _ =
				writeln!(o, "    theme={:?} val=@{} repr={:?}", s.str_at(theme), value, s.str_at(repr));
		}
	}
	...
	let _ = writeln!(o, "BLOB image-bytes={} fnv1a64={:016x}", s.blob.len(), fnv1a64(&s.blob));
	o
}
