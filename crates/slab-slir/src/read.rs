...
fn index_range(offset: u32, length: u32, total: usize, name: &str) -> Result<Range<usize>, String> {
    ...
    if end > total {
        return Err(format!(
            "{name}: range {start}..{end} exceeds pool length {total}"
        ));
    }
    Ok(start..end)
}
...
fn header(bytes: &[u8]) -> Result<&[u8], String> {
    if bytes.len() < 4 {
        return Err(format!(
            "truncated: need 4 bytes at offset 0, have {}",
            bytes.len()
        ));
    }
    if &bytes[..4] != b"SLIR" {
    ...
    if bytes.len() < 6 {
        return Err(format!(
            "truncated: need 2 bytes at offset 4, have {}",
            bytes.len() - 4
        ));
    }
    ...
    if major != crate::MAJOR {
        return Err(format!(
            "unsupported SLIR major version {major} (want {})",
            crate::MAJOR
        ));
    }
    if bytes.len() < 8 {
        return Err(format!(
            "truncated: need 2 bytes at offset 6, have {}",
            bytes.len() - 6
        ));
    }
    ...
    if minor > crate::MINOR {
        return Err(format!(
            "unsupported SLIR minor version {minor} (maximum {})",
            crate::MINOR
        ));
    }
    Ok(&bytes[8..])
...
fn from_pb(mut doc: pb::Doc) -> Result<Slir, String> {
    let mut slir = Slir {
        strs: std::mem::take(&mut doc.strs),
        ..Slir::default()
    };
    ...
    same_len(
        "aval",
        &[aval_tag.len(), aval_lo.len(), aval_hi.len(), aval_num.len()],
    )?;
    slir.avals = aval_tag
        .into_iter()
        .zip(aval_lo)
        .zip(aval_hi)
        .zip(aval_num)
        .map(|(((tag, lo), hi), _)| Aval {
            tag,
            payload: Aval::pair(lo, hi),
        })
        .collect();
    ...
    same_len(
        "tup_dyn",
        &[tup_dyn_tag.len(), tup_dyn_num.len(), tup_dyn_param.len()],
    )?;
    ...
    let font_default_adv = u16s(
        "font_default_adv",
        std::mem::take(&mut doc.font_default_adv),
    )?;
    ...
    for index in 0..font_count {
        let cmap_range = index_range(
            font_cmap_off[index],
            font_cmap_len[index],
            font_cmap_cp.len(),
            "font cmap",
        )?;
        let advance_range = index_range(
            font_cmap_off[index],
            font_cmap_len[index],
            font_adv.len(),
            "font advances",
    ...
    let anim_stop_attr_off = nonnegative(
        "anim_stop_attr_off",
        std::mem::take(&mut doc.anim_stop_attr_off),
    )?;
    let anim_stop_attr_len = nonnegative(
        "anim_stop_attr_len",
        std::mem::take(&mut doc.anim_stop_attr_len),
    )?;
    let stop_count = same_len(
        "animation stop",
        &[
            anim_stop_pos.len(),
    ...
    let animation_count = same_len(
        "animation",
        &[anim_name.len(), anim_stop_off.len(), anim_stop_len.len()],
    )?;
    ...
    let list_count = same_len(
        "list",
        &[list_param.len(), list_field_off.len(), list_field_len.len()],
    )?;
    ...
    let list_field_enum_off = nonnegative(
        "list_field_enum_off",
        std::mem::take(&mut doc.list_field_enum_off),
    )?;
    let list_field_enum_len = nonnegative(
        "list_field_enum_len",
        std::mem::take(&mut doc.list_field_enum_len),
    )?;
    ...
    for (index, &sub) in list_field_sub.iter().enumerate() {
        ...
        if is_list != (sub != 0) {
            return Err(format!(
                "list_field_sub: field {index} type/sub-schema mismatch"
            ));
        }
        if sub != 0 && usize::try_from(sub - 1).map_or(true, |row| row >= list_count) {
            return Err(format!(
                "list_field_sub: schema row {} is out of range",
                sub - 1
            ));
        }
    }
    ...
    let list_item_field_off = nonnegative(
        "list_item_field_off",
        std::mem::take(&mut doc.list_item_field_off),
    )?;
    let list_item_field_len = nonnegative(
        "list_item_field_len",
        std::mem::take(&mut doc.list_item_field_len),
    )?;
    let item_count = same_len(
        "list item",
        &[list_item_field_off.len(), list_item_field_len.len()],
    )?;
    ...
    slir.holes = pairs(
        ...
        std::mem::take(&mut doc.hole_node),
    )?;
    ...
    let signal_count = same_len(
        "signal",
        &[sign_name.len(), sign_node.len(), sign_trigger.len()],
    )?;
    ...
    let icon_count = same_len(
        "icon",
        &[icon_name.len(), icon_node.len(), icon_viewbox.len()],
    )?;
    let mut icons = Vec::with_capacity(icon_count);
    for index in 0..icon_count {
        let name = usize::try_from(icon_name[index])
            .map_err(|_| "icon_name: string index exceeds usize")?;
        ...
        if node >= slir.nodes.len() {
            return Err(format!(
                "icon_node: node {} is out of range",
                icon_node[index]
            ));
        }
        if slir.nodes.flags[node] & crate::flags::DETACHED == 0 {
            return Err(format!(
                "icon_node: node {} is not detached",
                icon_node[index]
            ));
        }
        icons.push(IconE {
